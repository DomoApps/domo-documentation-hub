...
name: add-to-nav
user-invocable: true
description: "add a page to docs.json navigation, insert an article into the nav, move a page to a different location in docs.json, register a new article in navigation, tag a nav group or section heading (e.g. Beta)"
...
### Tagging a group heading (e.g., Beta)

A group object supports an optional `"tag"` string that renders a small badge next to the dropdown heading in the sidebar — used most often to mark a whole multi-page section (such as an API reference) as `"Beta"`. Add it as a sibling of `"group"`:
...
When you tag a group this way, do **not** also put `tag: "Beta"` in the frontmatter of that group's Overview (landing) page — the badge would then appear twice in the same expanded section. Tag the heading only. See **Whole-Section Beta Features** in `Domo-KB-Style-Guide.mdx`.

**Caveat:** for API groups whose pages are auto-generated from an OpenAPI spec (the FileSets API group is one), the nav entry is regenerated by the OpenAPI sync workflow when the spec changes. A manually added group `tag` may not survive that regeneration — re-check after a spec sync and reapply if needed.
