...
excerpt: "JSON Webhook コネクターを設定し、Data Center で発行した Webhook URL を IFTTT や Github などのサードパーティアプリケーションに貼り付けて、JSON 形式のデータを DataSet にリアルタイムでプッシュします。"
---
...
このセクションでは、DomoとサードパーティアプリケーションでJSON Webhook接続を設定する方法について説明します。接続を適切に設定すれば、いつでも同じアカウントを使用して新しいJSON Webhook DataSetを作成できるようになります。コネクターアカウントは、Data Centerの**［アカウント］**タブで管理できます。このタブの詳細については、「[コネクターのユーザーアカウントを管理する](/s/article/360042926054)」を参照してください。
...
5. コピーしたリンクをサードパーティアプリケーションの適切な場所に貼り付けます。  
   ...
   次のスクリーンショットはGithubのものです。<img alt="json_webhook_3rd_party_url.png" src="/images/kb/ja/0EM5w000005wLkE.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/>
...
6. また、サードパーティアプリケーションでは、必ずコンテンツタイプを「application/json」に設定してください。<img alt="json_webhook_3rd_party_content_type.png" src="/images/kb/ja/0EM5w000005wLkD.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/>
7. 必要に応じて、サードパーティアプリケーションでほかの設定を行い、Webhookを保存します。
8. DomoのJSON Webhookコネクターインターフェースに戻ります。
9. （オプション）必要に応じて、**Webhookシークレット**を入力します。  
