...
5. Snowflakeの設定情報を入力します。
   ...
   - **［Snowflake接続URL］** — SnowflakeのURLです。これはSnowflakeログインページに表示されています。URLのフォーマットは次の通りです。\<組織名\>-\<アカウント名\>.snowflakecomputing.com  
     ...
     * **SQLワークシートから** — `SELECT CURRENT_ACCOUNT();`を実行するとアカウントロケーターが返され、`SELECT CURRENT_ORGANIZATION_NAME();`と`SELECT CURRENT_ACCOUNT_NAME();`を併せて実行すると、組織で修飾された識別子が返されます。
     ...
     {/* TODO[screenshot]: Snowflake Account Identifier — annotated example showing where the account identifier appears in the Snowsight URL and the Admin > Accounts page — see scripts/reports/screenshot_tracker.md */}
...
実稼働ワークロードの準備が整い、既存の機能的なウェアハウスを使用することと、Domo統合用の新しいウェアハウスを設定することのどちらがよいかを検討している場合は、Cloud Amplifierによる最適化についての [Domoブログの投稿 <i className="icon-arrow-square-out sm" aria-hidden="true" />](https://www.domo.com/blog/how-to-optimize-data-warehouse-strategy-with-cloud-amplifier/) を参照してください。
...
Cloud AmplifierとSnowflakeの統合で問題が発生した場合、以下の情報が役立つ可能性があります。 [Domoサポート](https://domo-support.domo.com) [<i className="icon-arrow-square-out sm" aria-hidden="true" />](https://docs.snowflake.com/ja/user-guide/key-pair-auth) にリクエストを送信することもできます。
