...
**Description**:  
Creates a new data stream using a connector and specifies configuration details. Usually creates a new DataSet.
...
| account              | Object | No       | Account information (e.g., ID). Can be omitted if no account is involved |
| updateMethod         | String | No       | Default is `REPLACE` , e.g., `APPEND`.                    |
...
```json
{
  ...
  "body": {
    "transport": {
      "type": "CONNECTOR",
      "description": "com.domo.connector.sampledata",
      "version": "1"
    },
    "configuration": [
        { "category": "METADATA", "name": "connectorName", "type": "string", "value": "Adobe Analytics" },
        { "category": "METADATA", "name": "connectorReport", "type": "string", "value": "Mobile Device Types" }
    ],
    "dataProvider": {
      "key": "sample-data"
    },
    "dataSource": {
      "name": "Some Sample Adobe Analytics Data",
    }
  }
}
```
