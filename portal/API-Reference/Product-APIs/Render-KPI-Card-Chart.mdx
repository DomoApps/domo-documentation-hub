...
Requests to this endpoint require authentication using an access token. The token must be included in the request header as `X-DOMO-Developer-Token`. For more information on generating an access token, refer to the [Domo Support Documentation](/s/article/360042934494?language=en_US).
