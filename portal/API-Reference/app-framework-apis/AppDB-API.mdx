...
<Note>
  Documents that contain DATE or DATETIME data must be formatted as YYYY-MM-DD
  or YYYY-MM-DDTHH:MM:SSZ in order to be successfully synced to a Domo DataSet.
</Note>

<Warning>
  Incorrect schema data types prevent DataSet creation. If your AppDB DataSet
  doesn't appear after 15 minutes, verify your schema uses valid types (STRING,
  LONG, DECIMAL, DOUBLE, DATE, DATETIME). Common mistakes include using NUMBER
  or INTEGER.
</Warning>

<Warning>
  Collections won't sync if the owner lacks permissions on the target DataSet.
  If a user is removed from Domo, their Collection stops syncing. The new owner
  must have both Collection ownership and Admin Permissions.
</Warning>
...
<Note>
  If you make any updates to the manifest and re-publish your app design, **you
  will need to edit the installed app and re-save it** for the manifest changes
  to take effect on the installed app (similar to what you would have to do when
  making changes to the mapping section of your manifest).
</Note>
...
Lists all documents from a given Collection. There is an upper limit of 10,000 documents per request. Use `limit` and `offset` to paginate through large collections.
...
| -------------- | ------- | -------- | ----------------------------------------------------------- |
| collectionName | String  | Required | The name given to the collection in the manifest            |
| limit          | Integer | Optional | Maximum number of documents to return. Default is 10,000.   |
| offset         | Integer | Optional | Number of documents to skip before returning. Default is 0. |

**HTTP Request**

```text
GET /domo/datastores/v1/collections/{collectionName}/documents/?limit={limit}&offset={offset} HTTP/1.1
Content-Type: application/json
Accept: application/json
```
...
domo
  .put(
    `/domo/datastores/v1/collections/Users/documents/b3ea3d2d-86c5-44e6-a2f4-985136bbbce1`,
    {
      content: { username: "Bill S. Preston, Esquire", Band: "Wyld Stallyns" },
    },
  )
  .then(console.log);
```
...
<Danger>
  **This is destructive and cannot be reversed**. Deleting documents in an AppDB
  Collection may have performance implications with regard to syncing to its
  corresponding DataSet in Domo if the `syncEnabled` property is set to `true`
  for the Collection.
</Danger>
...
```javascript
domo.delete(
  `/domo/datastores/v1/collections/Users/documents/1e61d99d-9885-419a-a33e-3be3941ee720`,
);
```
...
| unwind         | String  | Optional | Comma-separated list of fields to unwind (deconstructs an array field, producing one document per element). Must be combined with at least one of `count`, `groupby`, `avg`, `min`, `max`, or `sum`. |
| orderby        | String  | Optional | Field or alias to order by, followed by `ascending` or `descending`. Works with or without aggregation parameters. When used with aggregation, accepts aliases defined by other parameters; when used without, accepts field names in dot notation. |
...
Please refer to the official [mongoDB documentation](https://www.mongodb.com/docs/manual/reference/mql/update/) on Update Operators to better understand these operators and how to create them. Note that the operator `$setOnInsert` has no effect because AppDB partial updates do not perform upserts.

**Code Example**

```js
domo
  .put(`/domo/datastores/v1/collections/Users/documents/update`, body)
  .then((data) => console.log(data));
```
...
<Note>
  **Note:** Keep requests under approximately 700 document IDs (~26 KB URL) to avoid
  HTTP 414 URI Too Long errors from the framework proxy. For larger batches, split the
  IDs across multiple requests.
</Note>
...
<Danger>
  **This is destructive and cannot be reversed**. If you manually override the
  properties of a Collection that exists in your manifest, then any time an app
  Card is saved, the Card will revert back to the manifest definition of that
  Collection. It is best practice to only update Collections that you have
  created manually. Otherwise, to update a Collection, simply update the
  manifest.
</Danger>
...
<Danger>
  **This is destructive and cannot be reversed**. Deleting a Collection may have
  performance implications with regard to syncing to its corresponding DataSet
  in Domo if the `syncEnabled` property is set to `true` for the Collection.
</Danger>
...
Collections marked as `syncEnabled` are exported to the Domo instance every 15 minutes automatically. If you need to trigger an export immediately, call the export endpoint manually. Only collections that have a linked DataSet and pending changes are synced.

On success you receive a `200 OK` with an empty response body, meaning the export has been accepted and is running asynchronously. If an export is already actively in progress, you will receive `423 LOCKED` — retry once the current export completes.
...
The request body is optional. `domo.post()` returns an empty string (`""`) on success.
...
Returns an empty body. The export runs asynchronously after the response is returned.

By default, this endpoint exports the collections in the current card's own datastore. When `includeRelatedCollections=true`, it exports those collections **plus** the related collections this card instance is mapped to through the app's design (its `collectionsMapping`) — for example, a collection shared from another instance of the design.
...
const exportCollection = async () => {
  ...
  return resp.status; // 200 on success, 423 if export already in progress
};
```
