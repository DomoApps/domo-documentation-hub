...
Collections marked as `syncEnabled` are exported to the Domo instance every 15 minutes automatically. If you need to trigger an export immediately, call the export endpoint manually.

On success you receive a `200 OK` with an empty response body, meaning the export has been accepted and is running asynchronously. If an export is already actively in progress, you will receive `423 LOCKED` instead.
...
`domo.post()` returns an empty string (`""`) on success.
