...
<Warning>
Incorrect schema data types prevent DataSet creation. If your AppDB DataSet doesn't appear after 15 minutes, verify your schema uses valid types (STRING, LONG, DECIMAL, DOUBLE, DATE, DATETIME). Common mistakes include using NUMBER or INTEGER.
</Warning>

<Warning>
Collections won't sync if the owner lacks permissions on the target DataSet. If a user is removed from Domo, their Collection stops syncing. The new owner must have both Collection ownership and Admin Permissions.
</Warning>
...
Deleting documents in an AppDB Collection may have performance implications with regard to syncing to its corresponding DataSet in Domo if the `syncEnabled` property is set to `true` for the Collection.
</Danger>
...
<Danger>
**This is destructive and cannot be reversed**.
Deleting a Collection may have performance implications with regard to syncing to its corresponding DataSet in Domo if the `syncEnabled` property is set to `true` for the Collection.
</Danger>
