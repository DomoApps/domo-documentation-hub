...
| unwind         | String  | Optional | Comma-separated list of fields to unwind (flattens nested arrays — one row per array element). Must be combined with at least one of `count`, `groupby`, `avg`, `min`, `max`, or `sum`. |
