---
title: Data API
permalink: 8s3y9eldnjq8d-data-api
---
...
Perform a `groupby` with predefined date grains. By default, numerical columns are summed and unique entries of non-numerical columns are counted. Can be combined with an [aggregation](#aggregations) to override this default behavior.
...
Aggregates identical data into groups. By default, numerical columns are summed and unique entries of non-numerical columns are counted. Can be combined with an [aggregation](#aggregations) to override this default behavior. Multiple columns can be grouped by delimiting each with a comma.
...
You can control the format of the data returned from this endpoint by setting the request `Accept` header. If leveraging the [domo.js](/portal/Apps/App-Framework/Tools/domo-js) library, the same can be accomplished by passing a `format` option in the request. Please refer to the [domo.get()](/portal/Apps/App-Framework/Tools/domo-js#domo-get-url,-options) documentation for further details.
...
`array-of-objects` is the default value used by `domo.get` if you don't specify a format.
...
**Example Response**
You get the Base64 encoded file contents

```
UEsDBBQACAgIALmy/VwAAAAAAAAAAAAAAAATAAAAW0NvbnRlbnRfVHlwZXNdLnhtbK2Tz04CMRDGX6Xp1dCCB2PM7nJAPaqJ+ABjO8s29F86BeHt7S5gDCLhwGnSznzf98ukraYbZ9kaE5ngaz4RY87Qq6CNX9T8Y/48uueMMngNNnis+RaJT5tqvo1IrGg91bzLOT5ISapDByRCRF ...
```

#### array-of-arrays
This is the format accepted natively by the Domo Phoenix library.
...
<Warning>
  By adding the `useBeastMode=true` query string parameter to your data calls,
  some built-in query functionality is disabled.
</Warning>
...
<Warning>**The SQL API does not currently support page filters.**</Warning>
