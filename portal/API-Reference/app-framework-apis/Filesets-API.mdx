---
title: "Documents API"
permalink: 7e8654dedb1c8-file-set-api
---
...
| path      | String | Yes      | The path to the File within the Document Collection |
...
domo
  .get(
    `/domo/files/v1/filesets/${documentCollectionId}/path?path=${encodeURIComponent(
      filePath,
    )}`,
  )
...
```js
domo
  .get(`/domo/files/v1/filesets/${documentCollectionId}/files/${fileId}`)
  .then((result) => console.log(result))
  .catch((error) => console.error(`Error: ${error}`));
```
...
```js
fetch(`/domo/files/v1/filesets/${documentCollectionId}/files/${fileId}`)
  .then((response) => response.json())
  .then((result) => console.log(result))
  .catch((error) => console.error(`Error: ${error}`));
```
...
```js
fetch(`/domo/files/v1/filesets/${documentCollectionId}/files/${fileId}/download`)
...
```js
domo
  .post(`/domo/files/v1/filesets/${documentCollectionId}/query`, {
...
```js
fetch(`/domo/files/v1/filesets/${documentCollectionId}/query`, {
  method: "POST",
...
fetch(`/domo/files/v1/filesets/${documentCollectionId}/files`, {
  method: "POST",
  body: formdata,
})
  .then((response) => response.json())
...
domo
  .post(`/domo/files/v1/filesets/${documentCollectionId}/files/search`, {})
...
domo
  .post(
    `/domo/files/v1/filesets/${documentCollectionId}/files/search?directoryPath=reports&limit=20`,
    {
...
```js
// Example 1: List all files
fetch(`/domo/files/v1/filesets/${documentCollectionId}/files/search`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({}),
})
...
// Example 2: Advanced search with directory path and filters
fetch(
  `/domo/files/v1/filesets/${documentCollectionId}/files/search?directoryPath=reports&immediateChildren=true&limit=20`,
  {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
| --------- | ------ | -------- | --------------------------------------- |
| documentCollectionId | String | Yes      | The ID of the Document Collection                   |
| filePath  | String | Yes      | The path to the File within the Document Collection |
...
```js
domo
  .delete(`/domo/files/v1/filesets/${documentCollectionId}/files/${fileId}`)
  .then((result) => {
    if (result === 1 || (result && result.status === "success")) {
      console.log("File deleted successfully.");
    } else {
...
```js
domo
  .get(`/domo/files/v1/filesets/${documentCollectionId}`)
  .then((result) => console.log(result))
  .catch((error) => console.error(`Error: ${error}`));
```
...
```js
fetch(`/domo/files/v1/filesets/${documentCollectionId}`)
  .then((response) => response.json())
  .then((result) => console.log(result))
  .catch((error) => console.error(`Error: ${error}`));
```
...
| description | String | No       | The new description for the Document Collection |
...
```js
domo
  .post(`/domo/files/v1/filesets/${documentCollectionId}`, {
...
```js
domo
  .delete(`/domo/files/v1/filesets/${documentCollectionId}`)
  .then((result) => console.log(result))
  .catch((error) => console.error(`Error: ${error}`));
```
