...
description: Build a working inventory tracker to learn how AppDB stores flexible documents, queries them, updates fields atomically, aggregates server-side, and syncs to a DataSet for cards — no connector required.
---

This guide builds a working inventory tracker to teach the essentials of **AppDB**, Domo's built-in document store for app data. You'll define a collection, write and query documents, update them atomically, aggregate on the server, and sync the results to a DataSet you can build cards on — all without a connector or refresh cycle.
...
You don't have to choose permanently: any collection can **sync to a DataSet** when you're ready to report on it — the [last step](#sync-your-collection-to-a-dataset) does exactly that.
...
<Note>
  **Note:** The manifest controls the collection, not the API. If you later
  ...
  collection — add a field to its schema, enable sync to a DataSet — edit the
  manifest instead. See [Working with the
...
## Sync Your Collection to a DataSet

Your inventory data now lives inside the app. To chart it — a low-stock alert card, a units-by-category bar chart, a dashboard for the whole warehouse — the data needs to be a Domo **DataSet**. Turn on **sync**, and AppDB copies the collection's documents into a DataSet you can build cards on like any other.

Sync is a property of the collection, so set it in the manifest (the source of truth). Enabling sync requires a `schema` — the columns you list become the DataSet's columns, and only those top-level `content` fields are synced.

Update the `Inventory` entry in `manifest.json` by using the Resources pane or editing the file directly:
...
We chose to have the heterogeneous fields from earlier — `warranty`, `weight`, `tags` — not appear as columns in the DataSet, but you could include them. 

Domo syncs `syncEnabled` collections to their DataSets **every 15 minutes**. During development, trigger a sync on demand instead of waiting:

<CodeGroup>
```js domo.js v6
// domo.js v6
await domo.appdb.export(true); // `true` includes related collections as well, which includes any Collections in the manifest with a fixed UUID
console.log("sync started");
```

```js domo.js
domo.post("/domo/datastores/v1/export?includeRelatedCollections=true").then(() => console.log("sync started"));
// `true` includes related collections as well, which includes any Collections in the manifest with a fixed UUID
```

</CodeGroup>

After the sync runs, find the new DataSet under **Data** and build cards on it like any other DataSet.

<Warning>
**Watch these sync rules:**

- Only **top-level** keys in `content` sync. A nested object syncs as its JSON string, not as separate columns.
- Use only these column types: `STRING` · `LONG` · `DECIMAL` · `DOUBLE` · `DATE` · `DATETIME` — if it doesn't appear after 15 minutes, check your types first.
- Format `DATE` and `DATETIME` values as `YYYY-MM-DD` or `YYYY-MM-DDTHH:MM:SSZ`.

</Warning>

See [Sync Collections](/api-reference/appdb-api/sync-collections) for the full sync endpoint and [Working with the Manifest](/portal/API-Reference/app-framework-apis/AppDB-API#working-with-the-manifest) for schema details.
...
This walkthrough covered the core read/write operations — bulk create, query with operators, atomic update, server-side aggregation, and sync to a DataSet. A few more AppDB capabilities are worth exploring:
...
- **Full endpoint reference** — Every endpoint for documents, collections, sync, and permissions is listed on the [AppDB API Overview](/portal/API-Reference/app-framework-apis/AppDB-API).
