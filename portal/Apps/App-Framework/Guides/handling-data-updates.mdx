...
Add a `thumbnail.png` to your project root and then publish your design. The thumbnail must be exactly 300×300 pixels. You can use this one:
...
(function (domo) {
  ...
  function getData(alias) {
    COUNTER += 1;
    domo.get('/data/v1/' + alias + '?sum=total').then(function (data) {
      TOTAL = data[0].total;
      updateUI();
    });
  }
...
:::caution Deprecated method name
`domo.onDataUpdate()` (without the trailing `d`) was the v4 API and no longer exists in current versions of ryuu.js. Use `domo.onDataUpdated()`.
:::
