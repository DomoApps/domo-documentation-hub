...
[![npm version](https://img.shields.io/badge/npm-v5.1.4-blue?logo=npm)](https://www.npmjs.com/package/ryuu.js/v/5.1.4)
...
**Note:** `Domo.filterContainer()` is the deprecated alias for this method (see the [Migration Guide](#migration-guide)). Passing an **empty filter array** to clear filters does not behave identically on every surface: on a **Dashboard** the filters clear, but inside **App Studio** the retained App State can cause the previous filter state to persist rather than fully clear. When you need App Studio to start from a clean state, pair the embed URL parameter `overrideFilters=true` with an explicit filter event, and always send an empty filter state explicitly rather than relying on omission. See [Filtering Options › Additional App Studio URL parameter](/portal/embed/embed-in-sites-and-apps/filtering-options#additional-app-studio-url-parameter).

{/* [pm-input] Embed/Developer Docs (confirm owner) — confirm the exact difference in filter-clearing behavior between Dashboards and App Studio when calling requestFiltersUpdate/filterContainer with an empty array, and how App State retention interacts with overrideFilters. <provenance: synthesized from community forum reports; needs confirmation.> */}
