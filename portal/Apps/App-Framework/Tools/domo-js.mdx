...
[![npm version](https://img.shields.io/badge/npm-v6.0.0-blue?logo=npm)](https://www.npmjs.com/package/ryuu.js/v/6.0.0)
...
await domo.appdb.updateCollection('Users', {
  schema: { columns: [{ name: 'username', type: 'STRING' }, { name: 'role', type: 'STRING' }] },
  syncEnabled: true,
});
await domo.appdb.deleteCollection('Users');
...
const unsubscribe = domo.onBroadcast('selection:changed', (msg) => {
  console.log(`App ${msg.sourceAppId} selected:`, msg.payload);
...
```javascript
domo.onBroadcastOnce('init:ready', (msg) => {
  console.log('Coordinator is ready, starting work');
  beginProcessing(msg.payload.config);
});
```
...
```javascript
// Only listen to selection events from the main data table
domo.onBroadcastFrom('selection:changed', 'abc-123-def', (msg) => {
