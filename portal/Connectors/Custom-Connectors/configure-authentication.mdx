...
<Frame>
  ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/02/AuthNone.png)
</Frame>
...
It is not required, but if you would like to check your connection with your API endpoint before you run your report, you can write an authentication script. If you choose to do this, set _auth.authenticationSuccess()_ and _auth.authenticationFailed('`<<Insert your message>>`')_ appropriately.
...
2. Enter your username and password into the **Username** and **Password** fields. These will be securely stored. You can access them in your authentication script by calling _metadata.account.username_ and _metadata.account.password_.
...
The Connector Dev Studio provides a built-in function if you need to base-64 encode a string: _DOMO.b64EncodeUnicode(\<string>)_.

<Note>
**Note**
The code block needs to determine and set the authentication to either _auth.authenticationSuccess()_ or _auth.authenticationFailed('`<<Insert your message>>`')_.
</Note>

<Frame>
  ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/02/UserAuth.png)
</Frame>
...
1. Select **API Key**.
2. Enter your API key into the **API Key** field. It will be securely stored. You can access it in your authentication script by calling _metadata.account.apikey_.
3. Write your authentication script as required by your API endpoint.
4. Click **Run Script** to run your authentication script.
...
<Frame>
  ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/02/AuthAPIKey.png)
</Frame>
...
2. Enter the required data in the Authentication fields. You will need to get the necessary information from the API documentation you wish to use. All data will be securely stored. For your information:
   ...
   - The key defining the access token returned by your api needs to be _access_token_.
3. Click **Get Access Token**. Domo will perform the authentication process and securely store your access token. You can access your token in your authentication script by calling _metadata.account.accesstoken_.
...
<Frame>
  ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2017/03/17114227/UserOauth2.01.png)
</Frame>
...
<Frame>
  ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2017/03/24221012/Custom-Account.png)
</Frame>
