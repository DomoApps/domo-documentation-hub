...
permalink: wbwvwmq9rd714-filtering-options
---

All these filter types are layered in the sequence below:

<Frame>
  ![FilteringOptions1](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions1.png)
</Frame>
...
**Warning:** Browsers are expanding their 3rd party cookie blocks. The best practice is to use Programmatic Filters instead of PDP to [stay future-proof](/portal/embed/embed-in-sites-and-apps/programmatic-filtering).
...
<Frame>
  ![FilteringOptions2](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions2.png)
</Frame>

### Server-side Programmatic Filters

**Documentation:** [Programmatic Filtering](/portal/embed/embed-in-sites-and-apps/programmatic-filtering)
...
<Frame>
  ![FilteringOptions3](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions3.png)
</Frame>
...
<Frame>
  ![FilteringOptions4](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions4.png)
</Frame>
...
<Frame>
  ![Filtering Options
  5](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions5.png)
</Frame>
...
[Using Pfilters to Apply Filters from URL Query Parameters to Embedded Dashboards](/portal/embed/embed-in-sites-and-apps/url-parameters-in-embedded-content#pfilters)
...
<Frame>
  ![Filtering Options
  7](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions7.png)
</Frame>
...
### Filters and bricks in App Studio

**Definition:** When an App Studio page contains a brick (custom-code app, including a blank brick), page-level filters — whether applied by Pfilters, the JS API `/v1/filters/apply` event, or Programmatic Filters — reach the brick the same way they reach any card: a filter is matched against the brick's underlying DataSet by column name. A filter whose column does not exist on the brick's data has no effect on that brick.

**Use Case:** Pass values that map to a real DataSet column as filters. For values that are *not* a column filter (a configuration input, a selected mode, a deep-link value), pass them as `appData` so the brick's code can parse them directly, rather than forcing them through a column filter. See [Passing Filters and Parameters into App Studio Bricks](/portal/embed/embed-in-sites-and-apps/url-parameters-in-embedded-content#passing-filters-and-parameters-into-app-studio-bricks).

**Warning:** A value pushed as a Pfilter/JS API filter but not matching a column the brick queries can leave the brick unfiltered (with the black error bar) instead of applying as intended. Keep column filters and arbitrary brick inputs on separate channels.
...
**Documentation:** [Page Filters and Filter Views on Dashboards](/s/article/360042923914)
...
<Frame>
  ![FilteringOptions8](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions8.png)
</Frame>

### Interaction filters across cards

**Documentation:** [Apply Card-to-Card Interaction Filters](/s/article/360042923914#apply-card-to-card-interaction-filters)
...
<Frame>
  ![FilteringOptions9](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/FilteringOptions9.png)
</Frame>
