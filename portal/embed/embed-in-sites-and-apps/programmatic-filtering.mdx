...
<Frame>
  ![benefits](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/17132451/Benefits.png)
</Frame>
...
<Frame>
  ![diagram](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2021/05/progfilterdiagram.jpg)
</Frame>
...
- The JavaScript version uses a Node.js repository
  - <a
      href="https://github.com/domoinc/domo-node-embed-filters"
      target="_blank"
      rel="noopener"
    >
      JS sample code
...
- The ASP version is based on a .NET repository
  - <a
      href="https://github.com/domoinc/domo-asp-embed-filters"
      target="_blank"
      rel="noopener"
    >
      ASP.NET sample code
...
- Check out example Python code for Programmatic Filtering
  - [Python sample code](/portal/embed/embed-in-sites-and-apps/code-examples#python-code-example)
- Check out example PHP code for Programmatic Filtering
  - [PHP sample code](/portal/embed/embed-in-sites-and-apps/code-examples#php-code-example)
...
```html
<iframe
  src="http://localhost:3001/embed/item/1"
  width="1200"
  height="600"
  scrolling="no"
></iframe>
...
- Step 4 below will swap your domain with the Domo instance domain in the self-submitting HTML form
  <Frame>
    ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/17152248/1.png)
  </Frame>
...
**A.** Create an API client for the user who will serve as proxy for every other viewer. Learn how to get your [Domo Client ID and Secret](/s/article/000005240#create-api-clients).
...
- For more information about the CLIENT_ID and CLIENT_SECRET, see [API Authentication Overview](/portal/API-Reference/overview).

**B.** Copy the IDs for the cards or dashboards from the embed dialog in Domo using the "private" authenticated option:

<Frame>
  ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/17135105/sharemenu.png)
</Frame>

<Frame>
  ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/17135122/embed-dialog.png)
</Frame>

**C.** Create a file with a blank name and .env extension

- Customize the configuration settings with your own values

<Frame>
  ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/17134743/env.png)
</Frame>
...
<Frame>
  ![Terminal](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/17151720/terminal.png)
</Frame>
...
- Once you make filter changes to the users.js file, you will need to save the file, restart the express server, refresh the page, and then log back in.
  ...
  <Frame>
    ![](/images/dev/s3.amazonaws.com/development.domo.com/wp-content/uploads/2020/01/22134321/filter-1.png)
  </Frame>
...
This does not work because the embedded experience started outside of the instance. External viewers only have access to the single card or dashboard that has been embedded unless you choose to upgrade beyond the view experience to the edit experience as documented [here](/portal/embed/embed-in-sites-and-apps/embedded-capabilities)
...
<Frame>
  ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/LinkingAcrossEmbedded3.png)
</Frame>
...
In the [JavaScript example code](/portal/embed/embed-in-sites-and-apps/code-examples#javascript-code-example) there are 3 important places where a change needs to be made:

**`.env` file**

- <a
    href="https://github.com/domoinc/domo-node-embed-filters/blob/master/.env"
    target="_blank"
    rel="noopener"
  >
...
- Step 7 in the readme talks about changing EMBED_ID{X} to as many dashboards as needed in the experience
  ...
  <Frame>
    ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/LinkingAcrossEmbedded5.png)
  </Frame>

**host page**

- <a
    href="https://github.com/domoinc/domo-node-embed-filters/blob/master/sample.html"
    target="_blank"
    rel="noopener"
  >
...
- Line 25 shows the path to the host page. You would swap out localhost:3000 for your domain
  ...
  <Frame>
    ![](/images/dev/https://web-assets.domo.com/blog/wp-content/uploads/2022/08/LinkingAcrossEmbedded6.png)
  </Frame>
...
- <a
    href="https://github.com/domoinc/domo-node-embed-filters/blob/master/users.js"
    target="_blank"
    rel="noopener"
  >
    https://github.com/domoinc/domo-node-embed-filters/blob/master/users.js{' '}
  </a>
- Uncomment the code in lines 6-23 which already references embed IDs for each dashboard
  <Frame>
    ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2022/08/LinkingAcrossEmbedded7.png)
  </Frame>

### Programmatic Filters vs. Client-Side pfilters (Security)

---

Programmatic Filters and [pfilters](/portal/embed/embed-in-sites-and-apps/url-parameters-in-embedded-content#pfilters) both use the same `column` / `operand` / `values` pattern, but they differ in one critical way: **where the filter is applied and whether the viewer can change it.**

| | Programmatic Filters | pfilters (and JS API filters) |
| --- | --- | --- |
| Where applied | Server-side, before the embed token is issued | Client-side, in the URL or via the embed API |
| Viewer can see / change | No — baked into the signed embed token | Yes — visible in the URL and editable by inspecting the page |
| Use for | Enforced row-level security | Exploration and convenience filters only |

<Warning>
**Warning: pfilters are not a security boundary.** Because pfilters live in the URL (or are passed through the client-side JS API), a viewer can read them, edit them, or remove them entirely and see the unfiltered data the embed has access to. Never rely on pfilters to restrict which rows a viewer is allowed to see. For any per-user data restriction, apply the filter server-side with Programmatic Filters so it is enforced inside the signed embed token before the content ever reaches the browser.
</Warning>

For the full layered comparison of PDP, Programmatic Filters, JS API filters, pfilters, and in-product filters, see [Filtering Options](/portal/embed/embed-in-sites-and-apps/filtering-options).

### PDP Inheritance When Querying Datasets via API

---

Programmatic Filters personalize what a viewer sees in an *embedded card or dashboard*. They do not, by themselves, change what rows a *direct DataSet API query* returns. When you query a dataset through the API using the service account's credentials, the request is authorized as that service account (the client id), so Personalized Data Permissions (PDP) policies are evaluated against the service account — not against the individual embed viewer.

<Warning>
**Warning:** If you query a PDP-governed dataset via the DataSet API and then hand the results to individual viewers, every viewer receives the rows visible to the service account, not the subset their own PDP policy would allow. To keep per-user restriction, either apply the equivalent Programmatic Filter (or an explicit filter) to each request, or render the data through the embed flow rather than a raw dataset query.
</Warning>

{/* [pm-input] Embed/Developer Docs (confirm owner) — confirm whether PDP policies can be inherited per requesting embed viewer when querying a dataset through the API (rather than being evaluated against the service-account client id), and document the exact access/permission requirements for the /cards/embed/auth flow. <provenance: synthesized from community forum reports; needs confirmation.> */}
