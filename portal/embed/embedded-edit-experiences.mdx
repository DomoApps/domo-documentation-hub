---
title: 'Embed Edit Experience'
permalink: ed061f0c295c0-embedded-capabilities
---
...
<iframe
  src="https://player.vimeo.com/video/530952420?badge=0&autopause=0&player_id=0"
  title="Embed Edit Experiences"
  frameborder="0"
...
<Frame>
  ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2021/06/domo-identity-broker-3.png)
</Frame>
...
<Frame>
  ![](/images/dev/web-assets.domo.com/blog/wp-content/uploads/2021/08/Picture1-2.png)
</Frame>

#### Authenticating using a JWT (JSON Web Token)

JWT's can be sent to the Domo Identity Broker as a GET parameter (in the URL) or a POST parameter (in the post body). A quick way for validation is to send a URL parameter to the Identity Broker URL, followed by /JWT?token=\{token\}.

**Destination Parameter:**

You can also pass a destination parameter in the URL, which will determine which content type is loaded for specific users (assuming you want to load something beyond the default landing page).

**Page as a destination:** Pass `/page/{{page_ID}}` to the destination parameter.
...
**App Studio as destination:** Pass `/app-studio/{{app-studio id}}` to the destination parameter.

Example:

```text
https://modocorp-idp.domo.com/jwt?token={{token}}&destination=/app-studio/{{app-studio id}}
```

**App Studio View ID as destination:** Pass `/app-studio/{{app-studio id}}/pages/{{page id}}` to the destination parameter.

Example:

```text
https://modocorp-idp.domo.com/jwt?token={{token}}&destination=/app-studio/{{app-studio id}}/pages/{{page id}}
```
...
- Alternate email
  – A secondary contact email for the user.
- Role
  – The role of the user at each login. The role must match exactly a valid role in the Domo instance.
- Employee ID
  – Must be alphanumeric
- Hire Date
  – Format: YYYY-MM-DD
- Title
  – Example: Retail Team Lead
- Department
  – Example: Sales
- Location
...
- Locale – Sets the preferred language, metrics and formatting in Domo. Valid values include:
  ...
  - zh-Hans-SG
- Timezone
  – Example: America/Denver or Asia/Tokyo. For a full list of valid timezone settings, see this article: [timezones](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones)
- Group
  – A list of XML escaped strings

The payload must be signed using the provided secret and be encoded using an HS algorithm (we encourage HS256). A great site for learning more about JWT's, creating them for testing purposes and finding access to different code repositories for creating and signing JWT's is [jwt.io](https://jwt.io)
...
For more information also see the [Domo Knowledge Base](/s/topic/0TO5w000000ZanbGAC/domo-everywhere?language=en_US). In particular, this article on [Routing, Creation, and Mapping](/s/article/6523741250455?language=en_US) may be helpful.
