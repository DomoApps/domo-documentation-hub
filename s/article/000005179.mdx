...
2. Select **Add Trigger** > **< *trigger type>***.
   ...
   <Frame><img alt="" src="/images/kb/workflowTriggerMenu.png" style={{width: 599, height: 251}}/></Frame>
...
* **API trigger —** Choose the workflow version. Syntax is generated in JavaScript and Python. It contains a trigger ID. You can copy and paste the syntax into the API call. Any calls made with this trigger ID will be exclusively used for tracking purposes so you can identify which trigger initiated your workflow.
   ...
   * **Access Request trigger —** Initiate a workflow whenever someone requests access to a Card, Page, or App.
     ...
     2. Configure the Access Request Trigger. The Access Request Details provide data from the Request Access event. The event captures the Requestor ID and the ID of the entity (Card, Page or App) where access was requested. This data can be mapped to the starting parameters of the workflow that have the same data types. Please note mapping this data is optional.
        ...
        Please note that the workflow will run whenever a user requests access to the entity type selected in the trigger. For example, if you choose Card, the workflow will execute every time someone requests access to any card. The workflow runs under the user that created the trigger.
