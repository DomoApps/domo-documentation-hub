...
excerpt: "Use Domo's Credit Utilization interface to monitor used credits, credit balances, subscription details, and the Rate Card for your consumption plan."
---
...
## Manage Consumption From DataFlow Runs

Running DataFlows—including Magic ETL—consumes credits, and long-running or large executions increase that consumption. Use the [Credit Utilization](#credit-utilization) interface and the [Credit Usage report](#credit-usage-report) above to identify what is driving usage: the **entityType** and **entityId** fields let you trace consumption back to a specific DataFlow.

To limit how often connectors and DataFlows refresh—and cap the credits they can consume—configure [Data Refresh Permissions](/s/article/000005492).

{/* [pm-input] Dan Brinton — Confirm how Magic ETL credit consumption relates to processed row counts (for example, a runaway many-to-many join that generates very large row counts), how to cancel an in-progress DataFlow or ETL execution, and whether any automatic protections stop runaway runs. Synthesized from community forum reports; needs confirmation. */}
