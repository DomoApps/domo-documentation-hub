...
* Configure
  [write capability](/s/article/4402322966807#account-creation-—-write) for Snowflake with Cloud Integrations.
* For at least one Snowflake warehouse, choose **Transform** as the operation.
  + This MUST be on the same Cloud Integration configured with write capability.
      
    <Frame><img alt="transform operation.jpg" src="/images/kb/ka0Vq000000AMQf-00N5w00000Ri7BU-0EMVq000000w6eX.jpg"/></Frame>
* All input DataSets must be part of the same Snowflake Cloud Integration. A Magic ETL DataFlow with inputs from the Domo cloud cannot execute on the Snowflake cloud.
...
* The Snowflake role of the service account used in the Snowflake Cloud Integration needs SELECT, MODIFY, USAGE, and CREATE privileges on the tables you are referencing as inputs. This should be addressed by the commands run when you configure write capability.
...
You can use this feature in the Magic ETL interface after completing a Snowflake Cloud Integration transform configuration.

Access Magic ETL from the details page of any DataSet by selecting **Open With** > **Magic ETL**.

<Frame><img alt="open with magic.jpg" src="/images/kb/ka0Vq00000032LR-00N5w00000Ri7BU-0EMVq000000w57N.jpg"/></Frame>
...
When you configure a Snowflake Cloud Integration to allow transform operations, a dropdown displays in the Magic ETL editor, where you can choose where the ETL executes. The dropdown provides options for each of the available Snowflake clouds.

<Frame><img alt="dropdown.jpg" src="/images/kb/ka0Vq000000AMQf-00N5w00000Ri7BU-0EMVq000000w4o1.jpg"/></Frame>
When a Magic ETL on Snowflake DataFlow executes for the first time, a new table is created in the Snowflake database configured as part of the write setup of the Snowflake Cloud Integration. By default, all subsequent runs of the Magic ETL DataFlow are configured with the Replace update method, but you can choose to use the Append method. Learn about [DataSet update methods](https://domo-support.domo.com/s/article/360043430733?language=en_US).
