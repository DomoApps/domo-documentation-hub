...
<Warning>
  **Important:** Due to an AWS limitation - If the DataSet you choose has less than 100 rows, the
  system will automatically switch to non-batch mode. 
</Warning>
...
4. Then choose the Execution Preference.

<Frame>![Screenshot](/images/kb/etl_textGeneration4.jpg)</Frame>
<Note>
  **Note:** Batch vs. non-batch processing differ primarily in latency, cost, and use case.

Batch is designed for high-volume, asynchronous processing, execution times will vary.
Batch processing is generally more cost-effective.

Non-batch is intended for more consistent execution times for a small number of rows.
Non batch is always used when processing less than 100 rows of data.
</Note>
...
8. Then configure the desired output; Structure/Unstructured Outputs. Single column vs. multiple output columns, with name, type, description.​

<Frame>![Screenshot](/images/kb/etl_textGeneration8.jpg)</Frame>
<Frame>![Screenshot](/images/kb/etl_textGeneration9.jpg)</Frame>
9. Then choose the temperature of your desired response. Lower values provide
...
- Tokens​, Input/Output tokens​ vary based on Prompt & Data volume processed​
...
## AI Classification Tile <Badge className="text-primary bg-primary/10 font-bold">Beta</Badge>
...
<Frame>![Screenshot](/images/kb/classifyTile.jpg)</Frame>

Categorizes data (text, images, files) into predefined classes.

Providing a tile pre-configured for Classification enables the user to
focus on their classification entities and less on the configuration
details.

Faster and easier to configure & use than Text Generation for classification.
...
4. In the tile editor, choose the AI model you want to use with the Text
Generation tile.

<Frame>![Screenshot](/images/kb/etl_classification5.jpg)</Frame>
5. Then choose the Execution Preference.
...
7. Then configure the desired output; Single Label/Multi Label. 
Single label example: Fabric Type = Corduroy, Denim, Leather
    Fabric Type = Denim

Multiple Label example; Corduroy, Denim, Leather, Skinny, Flare one-hot encoded Boolean values
    Corduroy=True
    Denim=False
    Leather=False
    Skinny=False
...
## AI Sentiment Analysis Tile <Badge className="text-primary bg-primary/10 font-bold">Beta</Badge>
...
Automatically classify text (reviews, social media, surveys) as positive,
negative, or neutral. It enables businesses to monitor brand
reputation in real-time, improve customer service by identifying
frustrated users, and analyze trends for better decision-making.

Supported Values
Positive, Negative, Neutral, Not Applicable

Providing a tile pre-configured for Sentiment Analysis enables the
user to focus on their topics and less on the configuration details.

Faster and easier to configure & use than Text Generation for sentiment analysis.
...
7. Then configure the desired output; Overall sentiment or Targeted Sentiment. 
Overall Sentiment = Positive
Targeted Sentiment over multiple Topics; overall, fit, durability, customer service
