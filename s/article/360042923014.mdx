...
excerpt: "Optimize SQL DataFlows by adding indexes and limiting data to reduce timeouts and improve run efficiency on joins and WHERE clauses."
---
...
To understand how the database will run a query—and where it spends time—open a transform or output DataSet and select **Explain SQL** from the menu in the bottom-right corner. This shows the explain plan: an ordered list of the steps the database takes to execute the query, which you can use to find inefficient steps to optimize.

Explain SQL is available for both MySQL and Redshift SQL DataFlows.

<Note>
  **Note:** Explain plans are highly technical, so this feature is recommended for advanced SQL users only. Because MySQL determines which indexes improve run-time efficiency, not all of your index transforms will necessarily appear in the plan if MySQL determined that using them would be less efficient.
</Note>

## Convert to Magic ETL

If a SQL DataFlow remains slow or times out after you apply the optimizations above, consider rebuilding it in [Magic ETL](/s/article/What-is-Magic-ETL). Magic ETL is Domo's visual data-transformation engine and generally performs and scales better than SQL DataFlows for large or complex transformations.
