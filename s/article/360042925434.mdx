...
excerpt: "Group data into custom buckets with Beast Mode to create new categorical columns for filtering, sorting, and comparative analysis on Cards."
---

## Intro

This article explains how to group data into custom buckets with Beast Mode to create new categorical columns for filtering, sorting, and comparative analysis on Cards.
...
<Frame><img alt="Bucketing_basic.png" src="/images/kb/ka05w00000123SO-00N5w00000Ri7BU-0EM5w000005vOEa.png"/></Frame>
...
<Frame><img alt="Bucketing_advanced.png" src="/images/kb/ka05w00000123SO-00N5w00000Ri7BU-0EM5w000005vOEb.png"/></Frame>
...
My data has inconsistent values for the same thing—for example, `US`, `U.S.`, and `United States` all appear as separate values, which splits what should be one group across several bars. The strategy is to use a `CASE` statement to reclassify each set of related values into a single standardized label, then group on the new bucket. The Beast Mode calculation to create this bucket would look like this:

`` CASE WHEN `Country` IN ('US', 'U.S.', 'USA', 'United States') THEN 'United States' WHEN `Country` IN ('UK', 'U.K.', 'United Kingdom') THEN 'United Kingdom' ELSE `Country` END ``

Ending with `` ELSE `Country` `` keeps any value you haven't explicitly listed as-is, so only the variants you name are consolidated.