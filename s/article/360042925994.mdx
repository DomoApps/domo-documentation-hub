---
title: "Create an Alert for a DataSet"
excerpt: "Create a DataSet Alert with multiple trigger rules and customizable messages that include data from any column or other DataSets."
---
...
You manage DataSet Alerts in the Alert Center, just as with Card Alerts. There you can edit rules and messages, change your notification settings, see the alert history, share alerts with other users, and subscribe to other DataSet Alerts. For more information about the Alert Center, see [Alert Center Layout](/s/article/360043430373).'

---

## Create a DataSet Alert

You create DataSet Alerts in the Data Center, in the Details view for the DataSet. Click on **Alerts**in the details view tab bar to open the **Alerts** tab. This tab shows cards fBackor all Alerts that have been created on this DataSet. You can click any Alert to open a panel in which you can edit the Alert, see its history, share it, and so on. In this tab you can also click **New Alert** to open the 3-step Alert wizard, where you can configure your new Alert. The steps in the wizard are as follows:
...
If PDP policies have been set on this DataSet, you can specify how PDP is applied to Alerts in Step 1 of the wizard. For more information, see [PDP and Alerts](/s/article/360042934634).
...
<Frame>![dataset_alert_wizard_metric_menu.png](/images/kb/0EM5w000005vOc2.png)</Frame>
...
<Note>**Note:** The following DataSet types do not support row Alerts:</Note>
...
<Frame>![dataset_alert_wizard_filters_example.png](/images/kb/0EM5w000005vOc4.png)</Frame>
...
<Frame>![dataset_alert_wizard_message_with_macros.png](/images/kb/0EM5w000005vOc6.png)</Frame>
...
<Frame>
![dataset_alert_wizard_macro_list.png](/images/kb/0EM5w000005vOcB.png)
</Frame>
...
6. (Conditional) If you didn't customize your Alert message previously, you can do this by clicking the arrow next to **Webhook body**, scrolling down to the bottom of the body content, then clicking the pencil to open the message editor. For more information about the options here, see [Customizing your Alert message](#customize-an-alert-message).
...
You can cause a Task to be created when an Alert is triggered. You can choose the Project in which the new Task will be added, edit the customized Alert message if you haven't already, and select an owner for the new Task. As soon as the Alert threshold is reached, this new Task will be added to Domo and assigned to the owner you have selected. For more information, see [Projects and Tasks](https://domo-support.domo.com/s/topic/0TO5w000000Zao5GAC/projects-and-tasks?language=en_US).
...
1. Click the **Attach Action** button, which is located under the **Message** pane in the Alert Wizard.  
   ...
   <Frame>
   ![](/images/kb/0EM5w000005vOcD.png)
   </Frame>
2. Select **Task**.
3. In the **Project** menu, select the Project where this Task should live.   
   If the desired parent Project doesn't exist yet, you will need to create it. For more information, see [Creating a Project](/s/article/360043430333).
4. (Conditional) If you didn't customize your Alert message previously, you can do this here by updating the fields in the **Task name** area as necessary. For more information about the options here, see [Customizing your Alert message](#customize-an-alert-message).
5. Select the desired Task owner in the **Owner** menu.
6. Click **Next**to move onto the next step in the wizard.

<Note>**Note:** To create a Task from an Alert, you must select a valid Project in the **Project** menu — one that exists and that you have access to. If the Project you need doesn't exist yet, [create it](/s/article/360043430333) first. When the Alert triggers, the Task is created in that Project and assigned to the owner you selected.</Note>

**Troubleshooting:** If an Alert fires but no Task appears, verify that the Project you selected still exists and that you (the Alert owner) still have access to add Tasks to it, then re-save the Alert action.
...
### Scheduled Report Action

You can configure an alert to send a Scheduled Report when the alert triggers by attaching an action**.**You can attach an existing Scheduled Report and share it with people and groups within your organization.

#### Prerequisites

<Warning>**Important:** To send a Scheduled Report triggered by an alert you need three levels of access:</Warning>

- You must have access to the source dashboard or card that is referenced in the Scheduled Report.
- You must be the owner or recipient of the Scheduled Report.
- You must subscribe to the selected report.

Follow these steps to configure a Scheduled Report action:

1. In the alert wizard, select **Attach Action** in the left panel.  
     

   <Frame>![attach action.jpg](/images/kb/0EMVq0000012ban.jpg)</Frame>
2. Select **Scheduled Report**.  
     

   <Frame>![scheduled report.jpg](/images/kb/0EMVq0000012bfd.jpg)</Frame>
3. From the **Scheduled Reports** dropdown, choose the existing Scheduled Report you want to attach to the alert.
4. (Optional) Check the box labeled **Send to subscribers of this scheduled report (x people and x groups)** if you want to share the alert with people or groups who are currently subscribed to the existing Scheduled Report.
5. In the next field, search for and select people or groups in the instance or enter email addresses to share the alert with them.  
     

   <Frame>![Screenshot 2024-05-22 at 10.03.32 AM.png](/images/kb/0EMVq0000012cv3.jpg)</Frame>
6. Select **Save** to return to the alert wizard main screen. You can now see your scheduled report in the left panel under **Action**.  
     

   <Frame>![scheduled report action.jpg](/images/kb/0EMVq0000012dvx.jpg)</Frame>

## Share an Alert

In Step 3 of the wizard, you can share Alerts with other users and/or groups in your Domo so they receive notifications every time that Alert triggers. You can also write a custom message to let users know what is happening as well as delete users and groups who should no longer be able to see the Alert. You can delete users and groups only if you have an "Admin" default security profile or a custom role with "Manage All Groups" enabled. For more information about default security profiles, see [System Roles | Reference](/s/article/360043438953). For more information about custom roles, see [Manage User Roles and Grants](/s/article/360043438973).
...
Alerts are sent to the email address and telephone number specified in the user profile. For more information, see [Profile Page](https://domo-support.domo.com/s/topic/0TO5w000000ZaoLGAS/user-profile?language=en_US).

For information about configuring Notifications, see [Configuring Notification and Alert Settings](/s/article/360042925814).
...
1. In the Details view for the card with the Alert, click the <img alt="Screen_Shot_2020-04-16_at_2.44.09_PM.png" src="/images/kb/0EM5w000005vOc8.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> icon to show the Alerts for this card.
...
## Alert Reliability and Limitations

Keep the following in mind when you build DataSet Alerts:

- **Build alerts on a stored DataSet.** Domo evaluates DataSet Alerts against the data stored for the DataSet. Row Alerts are not supported on Views (or on Fusions, Appends, Upserts, Federated DataSets, or DataSets with more than 1M rows). For reliable alerting on data that comes from a View, set the Alert on the underlying stored DataSet — such as the output DataSet of the DataFlow that feeds the View — rather than on the View itself.
- **One notification per trigger.** When an Alert enters a triggered state, Domo sends a single notification for that trigger. If one evaluation matches multiple rows, those rows are reported together in that one notification rather than as a separate alert per row.

{/* [pm-input] Dan Brinton — Confirm how numeric column formatting and exponential/scientific display affect threshold evaluation — specifically whether the Alert evaluates the underlying stored numeric value rather than the formatted or exponentially displayed value. Synthesized from community forum reports; needs confirmation. */}

{/* [pm-input] Dan Brinton — Confirm how an attached Scheduled/Triggered Report resolves the actual triggered value versus the "Current alert value" placeholder in the report body. Synthesized from community forum reports; needs confirmation. */}

## Save an Alert

After you've selected the desired Alert components and entered a value, click **Save**.

Your new Alert now appears in the [Alert Center](/s/article/360043430373). You can do any of the following with this Alert:
