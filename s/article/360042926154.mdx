...
excerpt: "Best practices for naming, describing, owning, and organizing DataSets and DataFlows so they remain discoverable and maintainable."
---

## Intro

**Manage DataSet history:** The Managing DataSet History API provides information about the available data versions. See the [developer portal](https://developer.domo.com/portal/prt23r0zqw1vq-managing-dataset-history) to learn how to use it.
...
- Don't add date ranges in the DataSet name (for example, “Google Analytics 2021”). Most DataSets are set up on an automated schedule, and the name may become out of date soon. This practice will also help you avoid having to rename the DataSet and all other DataSets with dates in their names.
...
- When you need a second DataSet configured like an existing one, or need to recover an earlier configuration, use Domo's built-in DataSet options rather than rebuilding the connector settings by hand, which is time-consuming and error-prone.
...
- [Top 10 Dashboard Design Best Practices](/s/article/360043440173)
