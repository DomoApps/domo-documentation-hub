...
excerpt: "Use Domo's SurveyMonkey Advanced Connector to import survey responses and analysis data from your SurveyMonkey account into a DataSet via OAuth."
---
...
To connect your SurveyMonkey account and create a dataset, choose one of the available authentication methods.
* If you choose **Enterprise OAuth**, enter the **client ID** and **client secret** associated with your SurveyMonkey app, and then select the domain to use for API calls. In most cases, select **US**. If you are in Canada, select **Canada**. If you are in Europe, select **Europe**.
* If you choose **US-managed App** authentication method, select the domain to use for API calls. In most cases, select **.com**. If you are in Canada, select **.ca**. If you are in Europe, select **.eu**. 

Finally you must enter the username and password you use to log into your SurveyMonkey account.
...
The Domo SurveyMonkey connector uses OAuth to connect. Select the appropriate authentication method. 

If you select **Enterprise OAuth**, configure the following parameters: 

|  Field                    |  Description                                                                                                                                                                                                                                   |
|---------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Endpoint url domain       |   Select the domain to use for API calls. Most users use **US**. If you are in Canada, select **Canada**. If you are in Europe, select **Europe**. |
| Client ID                 |   Go to the SurveyMonkey API Help page and sign in to your SurveyMonkey account using the correct regional login. In the developer dashboard, open **My Apps**, select your app, and then go to **Settings** or **Credentials** to copy the client ID.  |
| Client Secret             |   Go to the SurveyMonkey API Help page and sign in to your SurveyMonkey account using the correct regional login. In the developer dashboard, open **My Apps**, select your app, and then go to **Settings** or **Credentials** to copy the client secret.  |

If you select **US-managed App**, configure the following parameter: 
|  Field                    |  Description                                                                                                                                                                                                                                   |
|---------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Endpoint url domain       |   Select the domain to use for API calls. Most users use **.com** domain. If you are in Canada, select **.ca**. If you are in Europe, select **.eu**. |

Click **Connect** (or select **Add Account** if you have existing SurveyMonkey accounts in Domo) to open the SurveyMonkey OAuth screen where you can enter your SurveyMonkey credentials. Once you have entered valid SurveyMonkey credentials, you can use the same account any time you go to create a new SurveyMonkey DataSet. You can manage connector accounts in the **Accounts** tab in the Data Center. For more information about this tab, see [Manage Connector/Adapter Accounts](https://domo-support.domo.com/s/article/360042926054?language=en_US "Manage Connector/Adapter Accounts").
