...
excerpt: "Domo's Snowflake Connector used username and password authentication, which Snowflake retired in November 2025. This connector is no longer functional—use the Snowflake OAuth, Enterprise OAuth, or Key Pair alternatives instead."
---
...
The retirement affects every Domo connector variant that authenticated to Snowflake with a username and password, not only the connector on this page. Any DataSet built on one of these connectors stops refreshing after the cutover and must be moved to an authentication method Snowflake still supports.
...
**To migrate a Snowflake DataSet off username/password authentication,**

1. Identify the DataSets that use a username/password Snowflake connector. These are the DataSets that began failing after the November 2025 cutover.
2. Choose a replacement connector:

   * Use an OAuth-based connector — [Snowflake OAuth Connector](/s/article/360061552054), [Snowflake Enterprise OAuth Connector](/s/article/000005534), or [Cloud Integrations](/s/article/4402322966807) — when you want to authenticate interactively or through your identity provider.
   * Use the [Snowflake Key Pair Authentication Connector](/s/article/360042931854) for unattended, service-account access that does not rely on an interactive login.
3. Create a new account for the replacement connector and authenticate with the new method.
4. Repoint each affected DataSet at the new connector account, keeping the same query, warehouse, database, and schema so the DataSet schema stays consistent.
5. Verify the new connection by running the DataSet manually and confirming a successful run in the Data Center. Once runs succeed, your existing update schedule resumes normally.
...
- The username and password for your Snowflake account. (Snowflake no longer accepts this authentication method — see the alternatives listed above.)
...
This connector has been retired. Snowflake discontinued username and password authentication in November 2025. Use the [Snowflake Enterprise OAuth Connector](/s/article/000005534), [Snowflake OAuth Connector](/s/article/360061552054), or [Snowflake Key Pair Authentication Connector](/s/article/360042931854) instead.
