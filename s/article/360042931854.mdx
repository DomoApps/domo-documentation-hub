...
1. Store the private and public key files in a secure local directory.  
2. Record the file paths for future reference.  
3. The private key uses the PKCS#8 format and is encrypted with the specified passphrase.

### Assign the Public Key to a Snowflake User

Run the following command to assign the public key:

ALTER USER example_user SET RSA_PUBLIC_KEY='MIIBIjANBgkqh...';

### Verify the Public Key Fingerprint

Retrieve the user's public key fingerprint:

1. DESC USER example_user;
SELECT SUBSTR(
  (SELECT "value" FROM $1 WHERE "property" = 'RSA_PUBLIC_KEY_FP'),
  LEN('SHA256:') + 1
) AS key;

2. Generate the fingerprint from the public key:
