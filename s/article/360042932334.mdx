...
<table border="1" cellpadding="1" cellspacing="1" data-aura-rendered-by="33:195;a"><thead><tr><th colspan="1" rowspan="1"><p><span class="s1"> Field </span></p></th><th colspan="1" rowspan="1"><p><span class="s1"> Description </span></p></th></tr></thead><tbody><tr><td colspan="1" rowspan="1"><p> Service Token JSON </p></td><td colspan="1" rowspan="1"><p> Paste the <strong>entire JSON object</strong> from the service-account key file you downloaded in Step 3: Creating a Service Account, in the "Prerequisites," above. Paste the file contents as-is, including the surrounding curly braces and every field (<code>type</code>, <code>project_id</code>, <code>private_key_id</code>, <code>private_key</code>, <code>client_email</code>, <code>client_id</code>, and so on). Do not paste only the <code>private_key</code> value or any other single field on its own. </p></td></tr></tbody></table>

<Warning>
**Warning:** A common cause of connection failures is pasting only the `private_key` value (or another single field) from the downloaded key file. The connector requires the complete JSON object exactly as Google generated it.
</Warning>

The downloaded key file looks similar to the following. Paste the whole block, including the outer `{ ... }`:
...
```json
{
  ...
  "private_key": "<your-full-PEM-encoded-private-key-block-here>",
