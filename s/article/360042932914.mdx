---
title: "InContact Echo Connector"
---

## Intro

InContact Echo is a survey system that measures customer satisfaction by collecting feedback immediately after calls. You can retrieve survey results from the InContact Echo API using Domo's [JSON Advanced connector](/s/article/360042932034). You can learn more about the InContact Echo API by visiting [http://echo.incontact.com/echo/help.taf?\_UserReference=7F00000146FFB91CDA7A004BDD05589A3AF8&topic=Echo%20API](http://echo.incontact.com/echo/help.taf?_UserReference=7F00000146FFB91CDA7A004BDD05589A3AF8&topic=Echo%20API).
...
1. Create a new [JSON Advanced connection](/s/article/360042932034) in Domo.
...
<Note>**Note:** To get the output as JSON, you must include the &outputtype=JSON parameter.</Note>

- In the **Do you require additional options?** dropdown, select **Yes**.
- In the **HTTPS Headers** field, enter the API key received from InContact.  
   For more information, see "Prerequisites," above.
- Click **Save**.  
     
   

  <Frame>![Screenshot](/images/kb/0EM5w000005vNwp.png)</Frame>
...
- The primary use of this API is to collect survey data. The endpoint for surveys is the [https://echo.incontact.com/company/api/echoapi.taf?classname=surveys&method=getSurveyResults&p1=101](https://echo.incontact.com/company/api/echoapi.taf?classname=surveys&method=getSurveyResults&p1=101) example. This is explained in the API documentation in the **SURVEYS/getSurveyResults** section. It is possible to grab multiple surveys in one API pull; this is explained in the **Requests** section of the API documentation.
