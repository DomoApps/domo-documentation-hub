...
excerpt: "Configure a Federated Data connection to render Domo cards against an external database without uploading the data into Domo."
---
...
Domo supports most commonly available databases. See the [list](#supported-databases) below.
...
\*We recommend using the new cloud-native federated experience for these providers. See the [Cloud Integrations documentation](https://domo-support.domo.com/s/topic/0TO5w000000Zjd3GAC/cloud-amplifier?language=en_US).
...
<Frame><img alt="IMAGE1.png" src="/images/kb/ka0Vq0000006bRV-00N5w00000Ri7BU-0EM5w000005vNun.png"/></Frame>
...
<Frame><img alt="IMAGE2.png" src="/images/kb/ka0Vq0000006bRV-00N5w00000Ri7BU-0EM5w000005vNuk.png"/></Frame>
...
The **Connect a Federated DataSet** modal displays. See [Configure New Account](#configure-new-account) below.

<Frame><img alt="federated.jpg" src="/images/kb/ka0Vq0000006bRV-00N5w00000Ri7BU-0EMVq000001Q58L.jpg"/></Frame>
...
<Note>
  **Note:** The connection detail fields vary by adapter. OAuth and federated-OAuth providers (such as Snowflake) prompt for additional provider-specific fields during account setup. For the providers marked with an asterisk in [Supported Databases](#supported-databases), Domo recommends the cloud-native federated experience — see the [Cloud Integrations documentation](https://domo-support.domo.com/s/topic/0TO5w000000Zjd3GAC/cloud-amplifier?language=en_US).
...
<Frame><img alt="IMAGE3.png" src="/images/kb/ka0Vq0000006bRV-00N5w00000Ri7BU-0EM5w000005vNui.png"/></Frame>

The Federated Agent option is ideal for databases that cannot allow access to queries from Domo. Typically, these are on-premise databases. Under this architecture, an intermediate agent is installed in a location that has access to the database. Typically within the firewall of the database. This agent uses a socket connection to Domo to capture the queries, connects to the database and executes the queries, and returns the results to Domo. A configuration file is used to manage the connection to Domo and the target database.
...
* Allowlisting (if applicable) must be completed
  ...
  + [Pusher's domain <i className="icon-arrow-square-out sm" aria-hidden="true" />](https://pusher.com/) needs to be allowlisted on your firewall(s).
      
    IP allowlisting is not available here. See Pusher's [documentation <i className="icon-arrow-square-out sm" aria-hidden="true" />](https://docs.bird.com/pusher/channels/channels/connecting/can-you-provide-me-with-a-list-of-ip-addresses-that-channels-uses) for more information
...
3. After setting up the federated agent, navigate to the Data Center to authorize the agent. In the left navigation, select <img src="/images/kb/ka0Vq0000006bRV-00N5w00000Ri7BU-0EMVq000001Q5Zl.jpg" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> **More** > **Federated Agent Management**.
    ...
    <Frame><img alt="management.jpg" src="/images/kb/ka0Vq0000006bRV-00N5w00000Ri7BU-0EMVq000001Q42e.jpg"/></Frame>
...
Results are cached at a DataSet & SQL query level. Every card, Custom App, and DataFlow produces SQL used to select results from an underlying DataSet. The combination of the produced SQL and the DataSet serves as a cache key. If multiple cards produce the same SQL to retrieve data from the same DataSet, they may use the same cache.

<Note>
**Note:** Queries that contain dynamic functions — such as NOW(), CURRENT_DATE(), or CURRENT_TIMESTAMP() — produce a different SQL string on every card load. Because the cache key is the exact SQL text paired with the DataSet, these queries generate a new cache key each time and effectively bypass caching. If cache efficiency matters, consider filtering by a date column in a Beast Mode formula instead of using dynamic functions directly in SQL.
</Note>

The same goes for multiple users that access a card or cards producing the same SQL against the same DataSet.
</Accordion>
...
<Accordion title="Does a Federated DataSet run an unfiltered query against my source database?">

When a card or query runs against a Federated DataSet, Domo may issue a broad, unfiltered query to the source database to retrieve and cache the underlying data before card- and page-level filters are applied. As a result, a single card load can scan more of the source table than the visible filters suggest, which can affect source resources on large tables. This preemptive query cannot be turned off, so plan capacity on your source database accordingly.

The cache TTL you set on the connection (see the caching FAQs above) determines how long those results can be reused before Domo queries the source again. A shorter TTL means more frequent queries against the source; a longer TTL reduces source load but serves older data.

{/* [pm-input] Tasleema Lallmamode — confirm the preemptive unfiltered-query/materialization behavior for Federated DataSets, that it cannot be prevented, and how the cache TTL interacts with it. synthesized from community forum reports; needs confirmation. */}
</Accordion>
</AccordionGroup>
