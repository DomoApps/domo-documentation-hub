---
title: "Old Magic ETL Tiles: Data Science"
---

### Intro

<Note>
  **Note:** This article is for legacy Magic ETL. For information on the new Magic ETL, see [Magic ETL](/s/topic/0TO5w000000ZanvGAC).
...
<Note>
  **Note:** To perform accurate and efficient data science analysis on your data, we recommend using clean data. See [Data Cleaning and Feature Engineering](https://en.wikipedia.org/wiki/Feature_engineering "https://en.wikipedia.org/wiki/Feature_engineering") for more information.
...
A snapshot of the "Catastrophic Test.xlsx [Wholesale\_Distributor\_Sales.xlsx](https://domosoftware.sharepoint.com/:x:/s/CS-Edu-PublicFiles/EZt5R1P6XTpCoHGOxOS5BKcBZN34yYrrrF7tTj8hN7ytGQ?e=5xPTZI "Wholesale_Distributor_Sales.xlsx")" DataSet is found below.
...
1. Add the Classification Data Science tile and connect it to your input DataSets.
...
5. Lastly, select either
   [Naïve Bayes](#h_01F0MEVR0PPQFM9TB077TE3JM6 "ETL Actions: Data Science") or [Random Forest](#h_01F0MEVX4PSQBDXCP08DZBSASN "ETL Actions: Data Science") as the algorithm powering the Classification tile.
...
The following example illustrates how the clustering algorithm can be implemented and used in Magic ETL in Domo. The sample DataSet: [Wholesale\_Distributor\_Sales.xlsx](https://domosoftware.sharepoint.com/:x:/s/CS-Edu-PublicFiles/EZt5R1P6XTpCoHGOxOS5BKcBZN34yYrrrF7tTj8hN7ytGQ?e=5xPTZI "Wholesale_Distributor_Sales.xlsx") (440 rows) can be downloaded if you would like to try this example.
...
<Note>
  **Note:** U sing too many clusters is typically not beneficial as the interpretations among the resulting clusters would be difficult.

</Note>

4. Then, select either
   [K-means](#h_01F0MEXGD3DB1VT720YW7P17T9 "ETL Actions: Data Science") or [K-medians](#h_01F0MEXNHNN2JWS6A586AD9RRG "ETL Actions: Data Science") as the algorithm powering this Clustering tile.
...
The following example illustrates how the ARIMA forecasting algorithm can be implemented and used in Magic ETL in Domo. The sample DataSet: [Daily\_Web\_Sales.xlsx](https://domosoftware.sharepoint.com/:x:/s/CS-Edu-PublicFiles/EU2e-GaRbRhChqHFRgCAOi8BPDiR2vKenemskHJlcY8GMA?e=AAx9SK) (171 rows) is a artificially generated DataSet that contains daily revenue totals.  It can be downloaded if you would like to try this example.
...
4. Select the number of rows back in time must be chosen to base the future predictions on. By default, all of the rows will be used. The name of the prediction column must also be named. In this case, the default 'prediction' is used.
...
The following example illustrates how the outlier detection algorithm can be implemented and used in Magic ETL in Domo. The sample DataSet: [Wholesale\_Distributor\_Sales.xlsx](https://domosoftware.sharepoint.com/:x:/s/CS-Edu-PublicFiles/EZt5R1P6XTpCoHGOxOS5BKcBZN34yYrrrF7tTj8hN7ytGQ?e=igdQ5f "Wholesale_Distributor_Sales.xlsx") (440 rows) can be downloaded if you would like to try this example.
...
4. Next, select either the
   [Standard Deviation](#h_01F0MF4QW8E8D0D3HAKJB7764R "ETL Actions: Data Science") or [Mean Absolute Deviation](#h_01F0MF4ZAPRZA4XXY1FBN2D477 "ETL Actions: Data Science") algorithm. In this example, we will select Standard Deviation.
...
The following example illustrates how the Regression prediction algorithm can be implemented and used in Magic ETL in Domo. The sample DataSets: [Catastrophic\_Train.xlsx](https://domosoftware.sharepoint.com/:x:/s/CS-Edu-PublicFiles/EYq2O7xGusFHr_lq0qP1LfgBudWLtjjOlY8EXl_xV9GgIg?e=pGi7R3 "Catastrophic_Train.xlsx") (800 rows) and [Catastrophic\_Test.xlsx](https://domosoftware.sharepoint.com/:x:/s/CS-Edu-PublicFiles/EWJT9HIPVXNAhdnrdnECGV0BJJCN5na5X0I5l6o1lg_dcQ?e=ou64D0 "Catastrophic_Test.xlsx") (200 rows) are artificially generated DataSets and can be downloaded if you would like to try this example. They contain data on insurance claims where the goal is to train a prediction algorithm which can accurately predict the number of claims for each row in a DataSet based on data in other columns.
...
5. Lastly, select either the
   [Linear Regression](#h_01F0MF9CVHERPTWNKBP33ME62Y "ETL Actions: Data Science") or [Random Forest](#h_01F0MF9HPJ3HE4CT9EPBY9289D "ETL Actions: Data Science") algorithm. In this example, we will select Linear Regression.
...
### Data Science Tiles in Magic ETL (video)
