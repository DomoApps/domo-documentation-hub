...
excerpt: "Create a recursive SQL DataFlow that uses its own output as an input so each run appends new rows to a growing historical DataSet."
---

## Intro

This article explains how to create a recursive (or snapshot) SQL DataFlow that uses its own output as an input so each run appends new rows to a growing historical DataSet.
