---
title: "Pivot Table"
excerpt: "Build a Pivot Table in Domo to summarize large DataSets across both rows and columns with nested groupings, subtotals, and intersecting measures."
---
...
<Warning>
  **Important:** If you see the message **Warning: Multiple results encountered for the same location**—with cells in your Pivot Table displaying as \*\*\*\*\*—more than one record exists for the same row-and-column combination, so Domo cannot resolve a single value for that cell.

  This most often happens when a value is not aggregated at the cell's level of detail—for example, an unaggregated or `FIXED` Beast Mode calculation, or a Beast Mode `CASE` statement whose result is not wrapped in an aggregate. It can also occur when a pill filter leaves more than one qualifying record per cell.

  To resolve it, make sure each measure resolves to a single value per row-and-column combination: aggregate the values used in your Beast Mode `CASE` statements, wrap the calculation in an aggregate such as `SUM()`, `MAX()`, or `MIN()`, and add enough row or column groupings (or adjust your filters) so that each cell maps to exactly one record.
...
Pivot Tables have Table Calculation functionality. Follow this link to learn about [Table Calculations](/s/article/360043429573#table-calculations).
...
* **General Border Properties —** There are several general properties for border width and color available for Pivot Tables. Learn about these [border properties](/s/article/360043429573).
    
  <Frame><img alt="Screenshot 2024-02-20 at 3.25.49 PM.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EMVq000000Nyhp.jpg"/></Frame>
...
* Header Row Border Properties — There are several header row properties for border width and color available for Pivot Tables. Learn about these
  [border properties](/s/article/360043429573).
...
2. Enter your desired label in the **Data Label** field.
   ...
   <Frame><img alt="SpecifyTableLabels.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000006u0H9.jpg"/></Frame>
...
2. Select your desired aggregation in the **Aggregation** field.
   ...
   <Frame><img alt="ChangeAggregationType.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000006u0Gp.jpg"/></Frame>
   <Tip>
...
<img src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP5P.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> indicates that these rows or columns are in the same order as the original DataSet.

<img src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP5L.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> indicates that these rows or columns are in alphanumeric order.

<img src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP5F.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> indicates that these rows or columns are in reverse alphanumeric order.
...
<Frame><img alt="pivot_table_example1.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP5G.png"/></Frame>
### Use Case #2
...
<Frame><img alt="1.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP5U.png"/></Frame>
...
<Frame><img alt="2.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP53.png"/></Frame>
...
<Frame><img alt="3.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP57.png"/></Frame>
<Note>
...
<Frame><img alt="4.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP5J.png"/></Frame>
...
<Frame><img alt="5.png" src="/images/kb/ka0Vq0000005oVd-00N5w00000Ri7BU-0EM5w000005vP54.png"/></Frame>
...
Keep the following in mind when building Pivot Tables:

* **Values populate the inner data pane, not a row axis.** Columns you drag into the **Values** region appear as the intersecting cells of the table. You cannot rotate a measure so that it displays down the left side as a row grouping—only dimension columns in the **Rows** region form the row groupings.
* **Beast Mode calculations are row-level.** Beast Mode measures are computed row by row, while totals and subtotals are computed across the whole grouping (subtotals) or DataSet (totals). As a result, a Beast Mode measure cannot create a new calculated total or subtotal row, and a Beast Mode result in a total/subtotal cell may not equal the sum of the rows above it. This is expected behavior—see the Beast Mode note under [Table Charts](/s/article/360043429573#configure-total-and-subtotal-options-for-individual-columns).
* **Suppress single-item subtotals.** To hide subtotal rows for groupings that contain only one item, select the **Subtotals** > **Suppress Single Item Subtotals** property (described under [Subtotals Properties](#subtotals-properties) above).
* **Dynamic pivoting is not native to Magic ETL.** Pivoting on column values that are unknown or change over time (a "dynamic pivot") is not supported by the Magic ETL Pivot tile, which requires you to specify the output columns. To pivot on unknown values, use a MySQL DataFlow or an R/Python scripting tile. Learn more about the [Magic ETL Pivot tile](/s/article/360044951294).
* **No bulk edit to swap a value column across cards.** There is no native way to replace a measure in the **Values** region across multiple Pivot Table cards at once. Each card must be edited individually in Analyzer.
* **HTML-link content is not supported and breaks expand/collapse.** Pivot Tables do not render HTML-link content the way Table (HTML/Mega) charts do. When a column—such as a Beast Mode field that outputs an HTML `<a>` link—contains HTML-link content, expand/collapse can stop working for the affected columns. Use a Table chart when you need clickable HTML links in cells.

{/* [pm-input] Chris Wright — confirm that Pivot Tables do not support HTML-link content and that including an HTML-link (e.g., Beast Mode `<a>`) column breaks the expand/collapse behavior for the affected columns. Synthesized from community forum reports; needs confirmation. */}
...
<AccordionGroup>
  <Accordion title="How Do I Build Trailing-Twelve-Month (TTM) or Annualized Columns?">
    A Pivot Table has no built-in trailing-twelve-month or annualized layout. Build the rolling window in your data first, then pivot on it: in Magic ETL, add a column that indexes each row's month relative to the reporting period (for example, a month-offset or period-index column), then use that index—or a Beast Mode calculation that references it—as the **Columns** grouping so each TTM or annualized bucket becomes its own Pivot Table column. For details on the pivot step, see the [Magic ETL Pivot tile](/s/article/360044951294).

    {/* [pm-input] Chris Wright — confirm and document the exact recipe for TTM/annualized financial columns in a Pivot Table (the Magic ETL month-index calculation and the Beast Mode expression that maps rows into trailing-twelve-month or annualized buckets). Synthesized from community forum reports; needs confirmation. */}
  </Accordion>
</AccordionGroup>