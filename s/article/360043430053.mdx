...
excerpt: "Answers to frequently asked questions about creating, certifying, and using Beast Mode calculations in Domo."
---

## Intro

This article answers frequently asked questions about Beast Mode calculations.
...
**How long are Beast Mode calculation fields for date functions cached?**

For cards and DataSets that use a Beast Mode calculation, Domo scans the query for date functions and, based on the the date function in the query, the results are cached for a shortened period of time (30 minutes). At that point, the cache expires and the query is re-run.
  
For DataSet Views, if you edit the view rather than viewing the **Data** tab, a new query is triggered.
...
**How do I calculate a duration in hours and minutes (HH:MM)?**

Convert each time to seconds with `TIME_TO_SEC`, subtract, then convert the result back to a time value with `SEC_TO_TIME`:
...
<Note>
  **Note:** `SEC_TO_TIME` is limited to 24 hours. If a duration can exceed 24 hours, it overflows and loops back to 0. For longer durations, calculate decimal hours instead (below).

</Note>

**How do I calculate elapsed time as decimal hours?**

Subtract the two timestamps as UNIX time (seconds) and divide by 3,600:

```
(UNIX_TIMESTAMP(`End_Time`) - UNIX_TIMESTAMP(`Start_Time`)) / 3600
```

Divide by 60 instead for elapsed minutes.

**Why is my total elapsed time too high when I sum a DATEDIFF (or duration) calculation?**

If the same duration value repeats across several rows (for example, one duration per ticket spread over many line-item rows), summing it multiplies the duration by the number of rows. Collapse the duplicates to a single value per group with `MIN` or `MAX` before aggregating:

```
MAX(DATEDIFF(`Closed_Date`, `Opened_Date`))
```

Because every row in the group holds the same difference, `MIN` and `MAX` both return that one value instead of adding it repeatedly.

{/* [pm-input] Phil Fuchs — confirm whether TIMESTAMPDIFF is supported in Beast Mode (it is not currently in the Functions Reference), and the recommended pattern for excluding business hours (not just weekend days) from an elapsed-time calculation. Synthesized from community forum reports; needs confirmation. */}
...
- **Beast Mode FIXED functions (recommended).** A FIXED function lets you compute a denominator that ignores the current row's grouping, so you can build share-of-total, percent-of-revenue, and market-share calculations directly in Beast Mode. For syntax and worked examples, see [Beast Mode FIXED Functions](/s/article/4408174643607 "Beast Mode FIXED Functions").
- **Percent-of-total data label.** If you only need to display the percent of total as a chart label (rather than calculate it), insert a data label:
  1. In Analyzer, open **Chart Properties**.
  2. In **Data Label Settings**, enter `%_PERCENT_OF_TOTAL` in the **Text** field.
...
**How do I sum or aggregate a true/false (boolean) column?**

Many boolean columns arrive from a connector as *strings* (`'true'`/`'false'`, or `'Y'`/`'N'`), so a plain `SUM` won't add them up. Convert the values to `1` and `0` with a `CASE`, then wrap it in `SUM`:

```
SUM(CASE WHEN `flag` = 'true' THEN 1 ELSE 0 END)
```

To aggregate as a logical OR — return `1` for a group if *any* row in it is true — use `MAX` instead of `SUM`:

```
MAX(CASE WHEN `flag` = 'true' THEN 1 ELSE 0 END)
```

{/* [pm-input] Phil Fuchs — Confirm whether SUM() on a natively boolean-typed column (not a string) counts the true values directly in Beast Mode (i.e., whether Domo treats a boolean true as 1). Synthesized from community forum reports; needs confirmation. */}

**Why is the AVG of my CASE statement lower than I expect?**

If you average a `CASE` statement that ends in `ELSE 0`, every non-matching row contributes a `0` to the average and drags the result down. To average only the rows that match your condition, omit the `ELSE` so non-matching rows return `NULL` — `AVG` ignores `NULL` values:

```
AVG(CASE WHEN `Status` = 'Won' THEN `Deal_Size` END)
```

With `ELSE 0`, the zeros are counted in the denominator; without it, only the flagged rows are.

**Why do I see a warning that my calculation "uses both aggregated and non-aggregated columns"?**

This appears when a calculation mixes an aggregated expression (such as `SUM()` or `COUNT()`) with a row-level column. It is a **warning, not a blocking error** — the calculation still saves and the card still renders. It is safe to ignore when the non-aggregated column resolves to a single value per group on the card, but it can signal duplicated rows or `*****` pivot errors when that column takes more than one value within a displayed group. For worked examples and fixes, see [Use Non-aggregated Columns in Aggregated Beast Modes](/s/article/000005559 "Use Non-aggregated Columns in Aggregated Beast Modes").
...
**How do I calculate a ratio or divide one column by another?**

Aggregate the numerator and denominator *before* you divide—wrap each side in `SUM()` and divide the sums:

```
SUM(`Numerator`) / SUM(`Denominator`)
```

Do **not** divide at the row level and then aggregate (for example `` SUM(`a` / `b`) `` or a bare `` `a` / `b` ``). Row-level division calculates a separate ratio for every record, so the aggregated result becomes the sum or average of those per-row ratios—which does not equal the ratio of the totals and often returns a wrong (or `0`) value. Surround each column name with backticks. To protect against a `NULL` or zero denominator, combine this with the divide-by-zero guard shown in the previous answer. To display the result as a percentage, format the field as a percent in the card's value-formatting options, or multiply by 100:
...
1. You must save the Beast Mode calculation to the DataSet.
2. The query must include the parameter "useBeastMode=true".
3. When you refer to a Beast Mode calculation in the query, refer to it by its ID (Format: calculation\_ac6f2ae5-a1a3-4eac-b7e6-cff4c0a9ed2d). If the calculation is aliased, you can refer to it by its alias in the query in the returned result set.
4. You can find Beast Mode calculation aliases using the Beast Mode report in the Domo Governance connector.
5. If the Beast Mode calculation is aggregated, then the aggregation statement can be left out of the query.
6. If the Beast Mode calculation is not aggregated, the aggregation statement must be included in the query.