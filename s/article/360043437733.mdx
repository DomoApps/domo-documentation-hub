...
excerpt: "Install and use the Domo CLI tool to upload large DataSets, manage users and groups, and run data management commands from a terminal."
---
...
<Warning>**Important:** Updating the schema or table definition of a DataSet that uses an upsert (update) key can drop that key. When the key is removed, subsequent CLI or API uploads append or replace rows instead of matching and updating existing rows. Confirm the upsert key is still in place after any schema change, and re-apply it if it was dropped.</Warning>

CLI actions are recorded in the Domo **Activity Log** under the account that authenticated the session—the user or access token used with the `connect` command.

{/* [pm-input] Phil Fuchs — confirm (1) the exact mechanism by which a schema/table update drops the upsert key on CLI/API DataSets and the supported way to preserve or re-apply it, and (2) how CLI actions are represented in the Activity Log (community reports indicate they are attributed to the authenticated user but not distinctly tagged as CLI). A separate CLI-specific audit log is a feature request. Synthesized from community forum reports; needs confirmation. */}
