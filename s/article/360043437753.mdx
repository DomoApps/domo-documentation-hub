---
title: "Data Assembler Usage Guidelines (Legacy)"
excerpt: "Legacy Data Assembler guidelines covering partition size, input ratio, update frequency, and other criteria for stable jobs."
---

<Warning>
  **Attention:** We recommend that you use the partition capabilities available in Magic ETL rather than Data Assembler. Learn more about [partitioning](/s/article/000005541) in Magic ETL.
...
## Access and Manage Data Assembler Jobs

<Note>
  **Note:** Data Assembler is a legacy tool. For new work, use the [partitioning](/s/article/000005541) capabilities in Magic ETL instead.

</Note>

### Create a New Job

When you create a new job, start with a manual schedule. After you confirm the job is ready for production, change the schedule as needed. Avoid triggers unless necessary—use the scheduling options first.

{/* [pm-input] Andrea Henderson — document how to open Data Assembler and create a new job (the app or page entry point and any required access token). Synthesized from community forum reports; needs confirmation. */}

### Edit an Existing Job

{/* [pm-input] Andrea Henderson — document how to access and edit an existing Data Assembler job's settings (where the job list lives and which settings can be changed). Synthesized from community forum reports; needs confirmation. */}