...
excerpt: "Schedule and email reports of cards or dashboards to people and groups at specified intervals, including external recipients."
---
...
<Tip>**Tip:** To control the layout and theming of a report email or to combine multiple cards into a single scheduled email, use [App Studio Report Builder](/s/article/000005829) instead of a standard scheduled report.</Tip>
...
Access scheduled reports from the main navigation by selecting **More** > **Scheduled Reports**.

On the **Scheduled Reports** screen, a navigation dropdown lets you switch between the different scheduled-report views available to your role (such as your subscribed reports, reports you own, the send history, and settings). Use this dropdown to move between views without leaving the page.

<Frame>![Screenshot 2024-08-13 at 4.43.05 PM.png](/images/kb/0EMVq000002CvXf.jpg)</Frame>

<Frame>![Scheduled Reports screen with the navigation dropdown open, showing the Dashboard, Card, and Report options.](/images/kb/scheduled-reports-navigation-dropdown.png)</Frame>

<Note>**Note:** The Reports view can appear empty until you choose a report type—dashboard or card—from the navigation dropdown. Select the type you're looking for to display its scheduled reports.</Note>
...
When planning a scheduled report, keep the following current limitations in mind:

- **Cadence —** Reports send on a daily, weekly, or monthly schedule only. Quarterly, yearly, and other custom intervals are not available.
- **Attachments —** For a *card* report, you can attach a CSV of the card's data (5 MB limit, card data only). Dashboard reports cannot include attachments, and PDF or Excel attachments are not available.
- **Card descriptions —** Card descriptions are not included in scheduled reports. If a card has a description, it doesn't display in a report for the card or a dashboard that contains the card.

{/* [pm-input] Ryan Despain — confirm these are not currently supported: reusable recipient distribution lists (recipients must be added individually as users/groups), bulk management of multiple scheduled reports at once, and a conditional "send only when data is available" option. Synthesized from community forum reports; needs confirmation. */}
...
2. Do the following depending on the type of report:  
   ...
   |  |  |
   | --- | --- |
   | Card report | Hover over the card until   <img alt="Wrench_Icon.png" src="/images/kb/ka0Vq000000Gos5-00N5w00000Ri7BU-0EM5w000005vOVV.png" style={{width: 50, height: 24, display: 'inline', verticalAlign: 'start', margin: '0'}}/>   **Card options** displays.  Or select **Card options**. The card options menu displays.  Or select **Schedule as Report**. The schedule report modal displays. |
...
12. (Optional) If you are scheduling a report for a card and you would like to send the underlying data for the card as a CSV attachment, check the box for **Include a spreadsheet of your data as an attachment (5MB limit)**.   
   ...
   <Frame>![spreadsheet_checkbox.png](/images/kb/0EM5w000005vOVw.png)</Frame>
...
When you choose to edit the schedule for a report, the edit dialog for the report opens. You can then change the name, schedule, and recipients as described in Scheduling a report, above.
...
<AccordionGroup>
  <Accordion title="Should I use a Scheduled Report or a Workflow for recurring emails?">
    Use a **Scheduled Report** when you want to email a card or dashboard visual on a daily, weekly, or monthly schedule. Scheduled reports can go to people inside or outside your organization (external recipients must be in your [authorized domains](/s/article/360042934534)), and a card report can include the underlying data as a CSV attachment.

    Use a **Workflow** when you need custom logic, conditional sending, or an email body assembled from a query result rather than a card image. In a workflow, you can use a [Query Table](/s/article/000005797#query-table) action to retrieve data and the [String Builder](/s/article/000005797#use-string-builder) to build the message body.

    <Note>**Note:** The Workflows **Email** User Task sends in-app emails to Domo users only. For sending outbound email to non-Domo recipients from a workflow, use a send-email/notification function. Learn more in the [Workflows reference](/s/article/000005797#send-email-and-notification-functions).</Note>
  </Accordion>

  <Accordion title="Can I send only a CSV of the data without the card image?">
    On a **card** report, check the box to include a CSV attachment of the card's underlying data (5 MB limit, card data only). The report email still contains the card image, which links back to Domo.

    {/* [pm-input] Ryan Despain — confirm whether a scheduled report can be delivered as a CSV attachment only, with no card image or link back to Domo in the email body. Synthesized from community forum reports; needs confirmation. */}
  </Accordion>

  <Accordion title="Can I turn the CSV attachment on or off when editing an existing report?">
    Yes, for a card report. When you edit the schedule, the edit dialog reopens with all of the report's current settings, including the **Include a spreadsheet of your data as an attachment** checkbox. Check or uncheck it and select **Schedule** to save. The attachment option is available for individual cards only, not dashboards.
  </Accordion>

  <Accordion title="How many reports can I schedule, and what happens if many are scheduled at the same time?">
    When multiple reports are scheduled around the same time, they compete for resources, which can cause queueing and longer overall processing times—see [Report Queueing](#caveats) in the Caveats above. Large or PDP-enabled reports add to this processing time.

    {/* [pm-input] Ryan Despain — confirm whether there is a documented limit on the number of scheduled reports per dashboard (or per instance) and the expected UI behavior when many reports are scheduled to run simultaneously. Synthesized from community forum reports; needs confirmation. */}
  </Accordion>
</AccordionGroup>