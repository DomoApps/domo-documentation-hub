...
excerpt: "Configure the Group By and Rank & Window tiles in Magic ETL to aggregate values and rank rows across DataSet columns."
---
...
<Frame>![Aggregate_Tab.png](/images/kb/0EM5w000005vNeY.png)</Frame>
...
<Frame>![Group_By.png](/images/kb/0EM5w000005vNeT.png)</Frame>
...
1. Inside Magic ETL, drag a **Group By** tile to the canvas from the **Aggregate** section in the left panel and connect it to your DataFlow.  
   ...
   <Frame>![group by tile on canvas.jpg](/images/kb/0EMVq000000Sdfx.jpg)</Frame>
2. (Optional) Rename the tile by selecting <img alt="" src="/images/kb/0EMVq000008iUU1.jpg" style={{width: 23, height: 23, display: 'inline', verticalAlign: 'start', margin: '0'}}/> **Edit** in the tile editor, then enter the name you want.
3. In the **Configuration** tab of the tile editor, select the column(s) to include in the grouping or add all columns.  
     

   <Frame>![Screenshot 2024-03-05 at 12.17.41 PM.png](/images/kb/0EMVq000000SdzJ.jpg)</Frame>
4. Enter a name for the new aggregated column.
5. Define how to aggregate the new column. Learn about the [aggregation types](#aggregation-types) below.
6. (Optional) You can choose to add another new column or add a formula.
   ...
   2. If you choose to add a formula, you can enter the formula or choose **Open the formula editor** to use the AI SQL Assistant to create your formula. Validate your formula and save and close to return to the canvas.
      ...
      **Use the AI SQL Assistant:** The AI SQL Assistant is available in the expanded formula editor. Use the prompt field at the bottom of the **Calculation** field to enter your query in natural language. The SQL appears above in the **Calculation** field.
      ...
      <Frame>![Screenshot](/images/kb/0EMVq000008iUQn.jpg)</Frame>

<Tip>**Tip:** You can preview the data transform by selecting</Tip>

<img alt="Screenshot 2024-03-05 at 12.24.12 PM.png" src="/images/kb/0EMVq000000Se49.jpg" style={{width: 20, height: 17, display: 'inline', verticalAlign: 'start', margin: '0'}}/> **Run Preview**. When the transform is complete, the results are available in the **Preview** tab of the tile editor.
...
| Combine distinct strings separated by , | Combines all of the DISTINCT text values in the column, separating them with commas. | Text |

#### Combine String Values (GROUP_CONCAT)

The **Combine strings separated by ,** and **Combine distinct strings separated by ,** aggregation types concatenate the text values in each group into a single comma-separated value. Like the underlying `GROUP_CONCAT` function, they skip null values, and a group with no matching rows returns null.

To combine only unique values, choose the **Combine distinct strings separated by ,** aggregation type. To use a separator other than a comma—or to build the concatenation as part of a larger calculation—add a formula in the Group By tile and call `GROUP_CONCAT` directly, specifying the separator:

```
GROUP_CONCAT(`Product` SEPARATOR ', ')
```

For the full function definition, see [Supported Functions in Magic ETL Reference Guide](/s/article/360044289573).

{/* [pm-input] Andrea Henderson — confirm whether GROUP_CONCAT(DISTINCT ...) is supported inside a Group By tile formula. The Supported Functions reference notes DISTINCT/ORDER BY are not supported in Magic ETL v2, while the SQL Expressions reference lists [DISTINCT]. synthesized from community forum reports and conflicting repo references; needs confirmation. */}

<Note>
  **Note:** Combining values across rows and returning a cross-row distinct result are functions of the Group By tile, not of a card Beast Mode. A Beast Mode cannot concatenate or return a distinct sum across rows—use the Group By tile for those results.
</Note>
...
<Frame>![Rank_and_Window.png](/images/kb/0EM5w000005vNec.png)</Frame>
...
For more information about all of the functions described in this section, see the [AWS docs](http://docs.aws.amazon.com/redshift/latest/dg/c_Window_functions.html) <img alt="external link.png" src="/images/kb/0EMVq000000T2OD.jpg" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> .
...
<Frame>![rank_and_window_partition.png](/images/kb/0EM5w000005vNeN.png)</Frame>

Note that the two amounts for Connecticut ("CT") are ranked first, followed by the five amounts for Delaware ("DE"), and so on.

### Ranking Functions

Ranking functions derive a ranking number for each value in a selected column and display it in a new column. There are three available ranking functions: [Rank](#rank), [Dense Rank](#dense-rank), and [Row Number](#row-number).
...
<Frame>![rank_and_window_rank.png](/images/kb/0EM5w000005vNeO.png)</Frame>
...
2. (Optional) At the top of the tile editor, rename the tile by selecting <i className="icon-pencil sm" aria-hidden="true" /> **Edit** and entering the name you want.
...
<Frame>![rank_and_window_dense_rank.png](/images/kb/0EM5w000005vNeP.png)</Frame>
...
The following simple example shows how this works. For this example, the user has selected the Sum function and has indicated a Preceding value of 1 and a Following value of 2. <img alt="rank_and_window_framing_example.png" src="/images/kb/0EM5w000005vNed.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> In the example, each value in the `Original Value` column is added to the one cell above it and the two cells below it, and the result appears in the same row in the `Derived Value` column. So for row 5, 3 is added to 1 (in row 4), 5 (in row 6), and 1 (in row 7). The total—10—appears in the `Derived Value` column in row 5. If there are not enough values above or below a given value to include in the calculation, these are omitted. For example, for row 7 there is only one Following value, so the equation simply becomes 5 + 1 + 2 = 8.

When partitioning is applied, partitions are honored in functions. In the following example, the user has selected Sum and indicated a Preceding value of 1 and a Following value of 2, just as in the previous example. However, they also designate the `Class` column as a partition. <img alt="rank_and_window_framing_example_partition.png" src="/images/kb/0EM5w000005vNeU.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> Because of the row grouping, due to the partition, some of the values are summed differently. For example, in row 4, only one Following value is available because the partition separates all "B" values into another group. So the equation for this row becomes 2 + 1 + 3 = 6. Likewise, for row 6, only the two Following values are available because of the partition; therefore, 5 + 1 + 2 = 8.

For an explanation of partitioning, see Understanding Partitions at the top of this section.

#### Average

The Average function takes the average of a given cell and its indicated Preceding and Following values. In the following example, the user has applied the Average function to the `Amount` column, set Preceding and Following values of 2, and set the `Cust` column as a partition. <img alt="rank_and_window_average.png" src="/images/kb/0EM5w000005vNeL.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> Follow these steps to configure the Average function:
...
The Count function returns a count of a given cell and its indicated Preceding and Following values. In the following example, the user has applied the Count function to the `Amount` column, set Preceding and Following values of 2, and set the `Cust` column as a partition. <img alt="rank_and_window_count.png" src="/images/kb/0EM5w000005vNeS.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> Follow these steps to configure the Count function:
...
The Sum function returns the sum of a given cell with its indicated Preceding and Following values. In the following example, the user has applied the Sum function to the `Amount` column, set Preceding and Following values of 2, and set the `Cust` column as a partition. <img alt="rank_and_window_sum.png" src="/images/kb/0EM5w000005vNef.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> Follow these steps to configure the Sum function:
...
3. Select **Add Function**.  
   ...
   <Frame>![add function.jpg](/images/kb/0EMVq000000T0pR.jpg)</Frame>
...
7. In the **What rows should be included**  field, enter the desired offset value.  
   ...
   <Frame>![Screenshot 2024-03-06 at 10.20.22 AM.png](/images/kb/0EMVq000000T1S9.jpg)</Frame>
8. Select **Apply** in the top right corner of the tile editor.  
   Three new steps appear.
...
<Frame>![rank_and_window_lead.png](/images/kb/0EM5w000005vNeW.png)</Frame>
...
1. Select the **Rank & Window** tile on the canvas to open the tile editor.
2. (Optional) At the top of the tile editor, reame the tile by selecting <i className="icon-pencil sm" aria-hidden="true" /> **Edit** and entering the name you want.
3. Select **Add Function**.
