...
3. Click
   **Details** to display additional settings.
   ...
   1. In the
      **Connection** tab, mark the **Allow big result sets** box.
      
      When selected, the driver does not set packet limits for results.
   2. In the
      **Cursors/Results** tab, mark the **Don't cache results of forward-only cursors** box and the **Force use of forward-only cursors** box.
...
1. Open
   **Settings** for the intended job.
2. Under
   **Settings > Impersonation,** turn on **Requires Impersonation**.
3. Enter the
...
1. Open
   **DataSet Jobs** in the **Accounts** pane in Workbench and locate the desired job.
2. Click
   **Source**.
3. Under
   **Transport Method**, enter the network file path in the **Network File Path** field.
...
1. Perform the initial load of the data using
   **Create New Data Source**.
   
   <Frame><img alt="WB_jobsetup.png" src="/images/kb/ka05w00000123rd-00N5w00000Ri7BU-0EM5w000005vPLO.png"/></Frame>
...
* Order by Ensure you have an **Order By** clause in the SQL statement, since it will be the value from the last row that will be saved in the Lastvalue variable. If you don't order by the column that contains the change data capture field, then you might not get the changed rows in the query.
...
<Frame><img alt="wb_mass_export1.png" src="/images/kb/ka05w00000123rd-00N5w00000Ri7BU-0EM5w000005vPLF.png"/></Frame>
