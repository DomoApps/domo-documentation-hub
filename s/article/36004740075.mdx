...
excerpt: "Use Jupyter Workspaces in Domo to develop notebooks against your DataSets and write transformed data back into Domo."
---
...
<Note>**Note:** Jupyter Workspaces is a premium feature that must first be provisioned for your instance. If the **Jupyter** feature does not appear in your **Admin** > **Features** menu, contact your Domo account team to request that it be enabled.

{/* [pm-input] Ken Boyer — Confirm the enablement-request process for non-standard or student instances, where Jupyter Workspaces may not be provisioned by default. Synthesized from community forum reports; needs confirmation. */}</Note>
...
<Warning>**Warning:** Deleting a workspace is permanent. There is no backup, and deleted workspaces and their notebooks cannot be recovered. Download or push any notebooks you want to keep before deleting.</Warning>
...
<Note>**Note:** Some pandas data types are not always written back to a Domo DataSet with the type you expect. If a column lands with an unexpected type or empty values, cast it to a string with `df['column'] = df['column'].astype(str)` before calling `write_dataframe`, or correct the column type in a downstream Magic ETL using a **Set Column Type** (Alter Columns) tile.
...
<Note>**Note:** To read credential values with `get_account_property_value`, the account's connector must allow it. When creating or editing the account, expand **show advanced options** and enable **Allow authorized users to access unencrypted credentials**. If this option is unchecked, `get_account_property_value` can return a 404 even though the account exists and is attached to the workspace. See the [Jupyter Troubleshooting Guide](/s/article/7440921035671) for more detail.</Note>
...
<Accordion title="What R version and kernels are available?">
Jupyter Workspaces offers Python and R kernels, which you select when you [create a workspace](#create-a-workspace). You can install additional packages from a workspace terminal — see [Install and Use Libraries](#install-and-use-libraries).

{/* [pm-input] Ken Boyer — Confirm the current R version shipped in Jupyter Workspaces (community reports cite R 4.1) and the list of supported/pre-installed kernels and packages. Synthesized from community forum reports; needs confirmation. */}
</Accordion>

<Accordion title="How do I search my workspace files for a code string?">
There is no cross-workspace search, but you can search the files within a running workspace from a notebook cell or terminal using standard Python. For example, walk the workspace directory and print files that contain a given string:

```python
import os

search_term = "read_dataframe"
for root, dirs, files in os.walk("."):
    for name in files:
        if name.endswith((".ipynb", ".py")):
