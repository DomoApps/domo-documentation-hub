...
excerpt: "Migrate a Workbench 5 service to a new server by exporting and importing the Workbench Registry and Encrypted Key between machines."
---
...
If you set up jobs manually on a new machine instead of migrating the Workbench Registry—for example, after a fresh reinstall—each new job creates a new Domo DataSet by default, which can leave you with a duplicate of a DataSet that already exists. To keep updating the existing DataSet instead, point the new job at it: on the job's **Overview** subtab, under **DataSet Name**, select **Browse**, then choose the existing DataSet from the **Select DataSet** dialog. The dialog lists the DataSets you have access to that were pushed from Workbench, and you can filter or search by name.
