...
1. **Create a default Snowflake database** — You need a Snowflake database that is exclusively for Domo to write Domo-managed tables. During setup, this database is the default.
   ...
   <Note>**Note:** Any tables not managed by Domo in this database will not be seen by the Snowflake Cloud Integration.</Note>
...
3. Select an existing Snowflake service account, or select **Add account** to create a new one.\
   This account authenticates to Snowflake and determines access to tables. For guidance on creating a service account, see [Creating a Snowflake Account](#creating-a-snowflake-account).
...
2. In the **Account Identifier** field, enter your Snowflake account identifier.\
   This is typically a subdomain of snowflakecomputing.com. You can find it in Snowflake under **Account > View Account Details**. See the [Snowflake account identifier documentation](https://docs.snowflake.com/en/user-guide/admin-account-identifier) for help.
...
2. Select the default **Database** and **Schema** where Domo will write data.\
...
3. In the **Default Role** field, enter the Snowflake role Domo will use for write and transform operations.\
...
5. In the **Temporary artifact storage** field, specify the database Domo will use for temporary storage during native transform execution.\
...
Once setup is complete, the integration's settings page shows **Write** and **Native Transform** as enabled.
...
OAuth allows Domo to query Snowflake data in the context of individual users rather than a shared service account, and is optional. When enabled, users accessing cards or previewing DataSets connected via OAuth will be prompted to authenticate with their Snowflake credentials.
