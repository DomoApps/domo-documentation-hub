...
Cloud Integrations enables you to connect Domo directly to your Snowflake data—without requiring data movement. Your data remains governed and secure in Snowflake, while Domo provides you with powerful tools to explore, transform, and visualize it. This guide walks you through setting up the integration, including read/write access and [OAuth configuration](#configure-oauth).

---

<Note>**Note:** Magic ETL leverages the Snowflake Cloud Integration, allowing you to effortlessly transform your data within Snowflake. Learn about [Magic ETL on Snowflake](/s/article/000005455).</Note>

---

## Architectural Overview

This image represents the integration between Cloud Integrations and Snowflake.

<Frame>![Screenshot](/images/kb/0EMVq00000DTRAD.jpg)</Frame>

## Prerequisites

Cloud Integrations setup consists of two parts: Read-only or Read/Write.
...
Before registering with Cloud Integrations for the Write portion of the setup, you must complete the following:
...
2. **Enter the Snowflake Account Identifier**  
   ...
   * **From a SQL worksheet** — Run `SELECT CURRENT_ACCOUNT();` to return the account locator, or `SELECT CURRENT_ORGANIZATION_NAME();` together with `SELECT CURRENT_ACCOUNT_NAME();` to return the organization-qualified identifier.
   ...
   <Frame>![Snowsight account details popover showing the account identifier (FJB74201), organization (SCBPTBU), cloud, region, edition, and locator.](/images/kb/snowflake-account-identifier.png)</Frame>

   For full details on the available identifier formats, refer to [Snowflake's account identifier documentation](https://docs.snowflake.com/en/user-guide/admin-account-identifier).
...
If you experience a problem with your Cloud Integration/Snowflake integration, the following information may help resolve the issue. You can also submit a request to [Domo Support](https://domo-support.domo.com/)[<img alt="" src="/images/kb/0EMVq00000DTPJi.jpg" style={{width: 14, height: 14, display: 'inline', verticalAlign: 'start', margin: '0'}}/>](https://docs.snowflake.com/en/user-guide/key-pair-auth).

**Missing Tables**

If you cannot find tables that your account has access to, ensure the table is materialized. Transient or Temporary tables on the Snowflake side cannot be used to create DataSets through Cloud Integrations.

**Setup Problems**

If you have problems setting up your Cloud Integration/Snowflake integration, follow the steps below:
...
<Accordion title="What are the differences between using connectors to create DataSets from Snowflake and using Cloud Integrations to create DataSets?">

- **Leaving data in Snowflake** — Perhaps the biggest difference is that all Snowflake connectors import/duplicate data from Snowflake to Domo, while Cloud Integrations leaves the data in Snowflake.
- **Bulk create** — Cloud Integrations allows you to look up and bulk select up multiple tables to immediately create multiple unique DataSets. To do this with connectors, you would have to configure each DataSet individually with a connector.
