...
excerpt: "Overview of relational database basics, Domo architecture, and data pipeline optimization techniques for working effectively with data in Domo."
---

## Intro

This article covers relational database basics, Domo's architecture, and data pipeline optimization techniques for working with data effectively in Domo.
...
### Combine DataSets of Differing Granularity

When two DataSets describe the same entities at different levels of granularity—for example, one row per transaction in one DataSet and one row per day in another—join them only after bringing both to a common granularity. Aggregating the finer-grained DataSet up to the coarser level before the join prevents the row-count inflation and mixed-attribution problems described in the common pitfalls above, and it keeps the data that enters a DataFlow small.

The recommended pattern is to aggregate first with a DataSet View, then join in Magic ETL:

1. Create a [DataSet View](/s/article/360046074774) on the finer-grained DataSet that groups and aggregates it up to the granularity you need (for example, sum sales to one row per region per day).
2. Use that View as the input to a Magic ETL DataFlow.
3. Join the aggregated View to the other DataSet in Magic ETL on the shared key.

Because the View has already reduced the finer set to the join granularity, the DataFlow reads far fewer rows than it would if it aggregated and joined the raw data in a single process.
