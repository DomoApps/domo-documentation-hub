---
title: "Workbench 5.1 FAQs"
excerpt: "Frequently asked questions about Domo Workbench 5.1, covering supported data sources, languages, behavior, and security."
---
...
<AccordionGroup>
<Accordion title="What is Workbench?">
...
<Frame>![workbench_architecture.png](/images/kb/ka05w00000123rd-00N5w00000Ri7BU-0EM5w000005vPLW.png)</Frame>
</Accordion>

<Accordion title="Can Workbench update or make any changes to our original data?">
...
Yes, both 32 and 64-bit Windows versions are supported. For more info on system requirements, see the [Workbench installation guide](/s/article/4406022964375).
</Accordion>
...
Workbench has no row limit for a single upload job. You can use a lastvalue variable to upload rows incrementally. For more information about query parameters, see [Understanding the Workbench 5.1 User Interface.](/s/article/4407032210199)
</Accordion>
...
For more information about running Workbench from the command line, see Running Workbench from the command line.
</Accordion>
...
1. Click the <i className="legacy-icon-database sm" aria-hidden="true" /> icon in the left-hand icon bar.
2. Double-click the name of the job you want to set up impersonation for.
3. Click the **Configure** subtab.
...
<Accordion title="Is there a way to update all of the jobs at one time?">

No. When migrating from a 4.x version of Workbench to a 5.x version, you need to migrate each job to the new version individually. This is because the encrypted information from Workbench 4.x must be re-entered and re-saved to encrypt the job details in Workbench 5.x.
</Accordion>
...
1. Click the **Configure** subtab. Then, click the **Edit** button under Processing.  
     
   <Frame>![blobid1.png](/images/kb/ka0Vq00000000o1-00N5w00000Ri7BU-0EM5w000005vPPW.png)</Frame>
...
* Where `"Customer\_ID" > '!{lastvalue:Customer\_ID}!'`  
  ...
  <Frame>![](/images/kb/ka0Vq00000000o1-00N5w00000Ri7BU-0EM5w000005vPPg.png)</Frame>
* Order by  
...
4. Click the <img src="/images/kb/ka0Vq00000000o1-00N5w00000Ri7BU-0EM5w000005vPPc.png" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> icon at the top of the pane to save your transform.
...
<Frame>![WB4_query.png](/images/kb/ka0Vq00000000o1-00N5w00000Ri7BU-0EM5w000005vPPl.png)</Frame>
...
When you first connect to Domo via Workbench, Workbench leverages your Windows system proxy settings. You can open your proxy settings for editing by clicking dots icon <i className="icon-dots-horizontal sm" aria-hidden="true" /> and selecting **Proxy**.
...
Yes. Workbench 5.1 allows the mass import/export of job files. Click the dots icon <i className="icon-dots-horizontal sm" aria-hidden="true" /> and select **Export**. You can then choose a save folder and select all of the jobs you want to export.

<Frame>![wb5_mass_export1.png](/images/kb/ka0Vq00000000o1-00N5w00000Ri7BU-0EM5w000005vPPa.png)</Frame>
</Accordion>
...
<Accordion title="What does it mean when Workbench says 'Live updates are not available'?">
...
<Accordion title="Can Workbench match filenames by 'contains' instead of an exact filename?">

Workbench does not support a "contains" match on filenames directly, but you can achieve the same result by pointing the job at a directory rather than a specific file. Workbench uploads any file that lands in that directory.

**To set up a path-only (directory) source:**

1. Create a new directory dedicated to the incoming file.
2. In the Workbench job, on the **Configure** tab, click **Edit** under **Source**.
3. For the **Path**, enter the directory only — do not include a filename.
4. Configure the rest of the job normally and save.

When a file appears in the directory, Workbench uploads it on the next run.

<Warning>
**Warning:** Every file that lands in the directory must have a schema identical to the file uploaded previously. If schemas differ, the job fails or produces unexpected results.
</Warning>

<Note>
**Note:** Only one file should reside in the directory at a time. Workbench uploads every file present in the directory on each run, so if multiple files are waiting, all of them are uploaded. Use Windows Task Scheduler (or similar) to delete or move each file after it has been uploaded.
</Note>
</Accordion>
</AccordionGroup>