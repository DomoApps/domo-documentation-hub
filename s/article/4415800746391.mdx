...
excerpt: "Use the PDP Automation tool in the Governance Toolkit to configure DataSet-driven PDP rules that run on a schedule or when the configuration DataSet updates."
---
...
<Note>
**Note:** PDP Automation configures **row-filter** policies (which rows each user or group can see). It does not automate **column policies** (column masking); those must be configured manually on each DataSet. Learn more about [column policies](/s/article/360042934614).
</Note>
...
* custom\_filter
  ...
  + When using between, the two values must be separated by '&'. For example: 04012020 & 04022020 | 04032020 & 04042020
      ...
      <Frame><img alt="Screenshot 2023-05-24 at 5.27.13 PM.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000006uq7Q.jpg"/></Frame>
      <Frame><img alt="Screenshot 2024-02-08 at 2.03.04 PM[8].jpg" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EMVq000000OOXR.jpg"/></Frame>
...
3. Fill out the following information:
  ...
  <Frame><img alt="PDP_Automation_Notification.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNox.png"/></Frame>
  <Frame><img alt="PDP_Automation_5.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNor.png"/></Frame>
...
<Frame><img alt="PDP_Automation_3.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNoz.png"/></Frame>
## Example Job Run

* PDP Before run:
    
  <Frame><img alt="PDP_Automation_1.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNop.png"/></Frame>
* Configuration DataSet:
    
  <Frame><img alt="PDP_Automation_Config.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNon.png"/></Frame>
* PDP after run:
    
  <Frame><img alt="PDP_Automation_2.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNos.png"/></Frame>
* Logging DataSet:
    
  <Frame><img alt="PDP_Automation_Metrics.png" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EM5w000005vNoq.png"/></Frame>
## Use All Rows Policy
...
* The attribute must be set to "managed." Learn about
  [attribute management](/s/article/000005164#attribute-management).
...
{/* [pm-input] Ryan Despain — confirm whether the attribute key entered in the `Value` column is case-sensitive (must match the Key exactly as defined on the Attributes page). Synthesized from community forum reports; needs confirmation. */}

<Frame><img alt="store in value.jpg" src="/images/kb/ka0Vq00000044g9-00N5w00000Ri7BU-0EMVq000003WBhl.jpg"/></Frame>
...
## Assign One User Multiple Values

To give a single user or group access to more than one value of the same attribute, enter all of the values in the `value` column of that policy row, separated by a pipe (`|`). Keep the `target_ds`, `policy_name`, `policy_column`, and `user_group` the same across that user's row so the values all apply to one policy.

For example, to give `jsmith@example.com` access to the `West`, `East`, and `Central` regions on a single policy, set that user's `policy_column` to `region` and set the `value` column of that row to:

```
West|East|Central
```

The user then has access to rows where `region` is `West`, `East`, or `Central`.

## Remote Instance Configuration

Remote instance configuration allows you to be in one Domo instance and configure PDP for other instances to which you have access. To configure PDP remotely, you need a [configuration DataSet](#configuration-dataset), which is described previously in this article. You also need a [Domo Access Token account](#configure-domo-access-token-account) configured with either a [Domo Access token](#generate-access-token) or with a username and password for the remote instance.

### Configure PDP Automation for Remote Instance

1. While creating or editing a job for PDP Automation, in the
   **Configuration** section, select a configuration DataSet populated with valid values for the remote instance according to PDP Automation guidelines given in this article. See [Configuration DataSet](#configuration-dataset).
2. Still in the
...
4. From the
   **Select the account for your instance:** list, select a valid [Domo Access Token account](#configure-domo-access-token-account). To learn how to configure a Domo Access Token account, see the next section.
...
2. In the sidebar, select
   <img src="/images/kb/ka05w00000129aT-00N5w00000Ri7BU-0EM5w000006uFA5.jpg" style={{width: 20, height: 20, display: 'inline', verticalAlign: 'start', margin: '0'}}/> **Accounts**.
3. Select
   **Add New** > **Account**.
...
3. On the
   **Access tokens** page, select **+ Generate access token**.
   ...
   <Frame><img alt="generate token.jpeg" src="/images/kb/ka05w00000128xR-00N5w00000Ri7BU-0EM5w000006uFA0.jpg"/></Frame>
