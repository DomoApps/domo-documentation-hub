...
excerpt: "Reference of the default Python and R libraries installed in the Domo 1.1 environment for Magic ETL Python and R scripting tiles."
---
...
The scripting tile's **Environment** selector controls which set of Python and R libraries your script can import. Two kinds of environment can appear in the list:

* **Domo 1.1** — the default environment, available in every instance. It includes over 400 Python and R packages; the full list is in the [Default environment page list](#default-environment-page-list) section below.
* **Custom environments** — any environments provisioned for your instance through Domo Support. A custom environment contains only the packages specified when it was requested and does *not* automatically include the Domo 1.1 packages, so a script that relies on a default package must name that package in the request. See [Requesting Custom Environments](/s/article/4662034775319).

To confirm exactly which libraries a given environment provides, select it, then open the **Packages** list in the scripting tile and search for the package you need.
