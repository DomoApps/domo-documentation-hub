...
Data Models is Domo's semantic layer offering. Instead of recreating the same joins in DataFlow after DataFlow—or duplicating the same calculated columns across many DataSets—you define the relationships between your DataSets once in a central model and reuse that model everywhere in Domo. This keeps metrics consistent and speeds up analysis. Data Models is found under **Features** > **Data Models** in the **Semantic Layer** section of the navigation menu.
...
Data Models is a beta feature, so some capabilities available for other Domo assets are not yet supported and a few errors are still being resolved. Keep the following limitations in mind:

- **All tables must share one warehouse.** Every table in a model must come from the same warehouse—you cannot combine Domo DataSets and Snowflake tables in a single model. Within that warehouse, you can still relate DataSets from disparate source systems through shared keys.
- **Joins are resolved at query time.** Because relationships are evaluated when a card runs, performance depends on the size of the underlying DataSets and the complexity of the model's relationships.

**Deploy a Model Across Environments**
...
<Accordion title="I get a Bad Request error when working with a Data Model.">
  The Bad Request error is a known issue in the current beta.
  {/* [pm-input] Ken Boyer — Confirm the trigger conditions and any workaround for the "Bad Request" error affecting Data Models. Synthesized from community forum reports; needs confirmation. */}
</Accordion>

<Accordion title="Filtering by a date field or configuring a summary number returns an error in Analyzer.">
  Using a date field from a Data Model to filter, or to configure a summary number or Graph By, is a known limitation in the current beta.
  {/* [pm-input] Ken Boyer — Confirm the Analyzer date-field errors reported for Data Models (filter-by-date and summary number / Graph By) and document how to set up date fields in the model so they work in Analyzer. Synthesized from community forum reports; needs confirmation. */}
</Accordion>
