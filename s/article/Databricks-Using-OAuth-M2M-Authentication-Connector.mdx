...
<Note>
**Note:** This connector uses OAuth machine-to-machine (M2M) authentication with a Databricks service principal, so it authenticates non-interactively — no person needs to sign in when a job runs. Use this connector when you need a service-principal (non-interactive) connection for scheduled, unattended Databricks jobs, rather than an interactive user login or a personal access token (PAT).
</Note>
