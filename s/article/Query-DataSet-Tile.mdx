...
{/* TODO[content]: Two unresolved items from Asana 1210394972306471 — (1) confirm the in-UI field label is "# of Loads" (was previously "# of Parallel Loads"). The original screenshot still showed the old label and the team had planned a new capture. (2) Specimen error messages for socket timeout and "exceeds maximum heap size of 16 GB" were not provided in the source draft and are stubbed below — replace once Joseph Peterson supplies the exact strings. See scripts/reports/screenshot_tracker.md. */}

## Intro

The **Query DataSet** tile lets you write a SQL query against a Databricks DataSet as the first step of a Magic ETL DataFlow, in place of the standard **Input DataSet** tile. It loads only the rows your query returns and can split the result into smaller batches, which improves performance and can prevent timeout and out-of-memory errors when working with large Cloud Amplifier DataSets.

Although the **Input DataSet** tile is the most common first step in Magic ETL, other tiles can be used in its place. The **SQL** tile, for example, lets you write a custom `SELECT` statement. The **Query DataSet** tile goes further by also subdividing the result set into smaller loads — useful when a single load would exceed the underlying Cloud Data Warehouse's resource limits.

Although the **Query DataSet** tile can be used with various data sources, it is strongly recommended for Databricks. Proper use of this tile can resolve large-DataSet errors such as:

- Socket timeout errors when the underlying query takes too long to return.
- "Exceeds maximum heap size of 16 GB" errors when a single load is too large for the CDW to process.

{/* TODO[content]: Replace the two bullets above with the exact error strings once provided. */}
...
<Warning>
  **Important:** The column you choose as the **Query partition key** must not contain `NULL` values. Rows whose partition-key value is `NULL` are ignored. Only choose a column that is guaranteed to be non-null for every row.
</Warning>
...
{/* TODO[screenshot]: Magic ETL Actions pane with the DataSets section expanded and the Query DataSet tile highlighted — see scripts/reports/screenshot_tracker.md */}
...
4. Configure the **Query partition key** — select the column the tile uses to split the result set into batches.
...
5. Configure **# of Loads** — the number of batches into which the result set is divided. See [Size Your Loads](#size-your-loads) for the recommended ceiling.
   ...
   {/* TODO[screenshot]: Query DataSet tile configuration panel with Query partition key and # of Loads fields populated. Confirm the in-UI label is "# of Loads" (was previously "# of Parallel Loads"). See scripts/reports/screenshot_tracker.md */}
...
To avoid bottlenecks, the largest batch must be small enough to fit within your Cloud Data Warehouse's resource limits. Batches are not guaranteed to be of equal size by row count or by data volume — the distribution depends on the partition-key column you choose.
...
6. **Count distinct values in the candidate column:**
...
10. **Compute the minimum `# of Loads`** required to keep the largest batch under 4 GB (round up):
...
A good partition-key column:

- Contains no `NULL` values (required — rows with `NULL` partition-key values are skipped).
