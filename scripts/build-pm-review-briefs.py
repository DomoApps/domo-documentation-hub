...
(the .docx step is skipped with a note if pandoc is not installed). It also seeds
PM-REVIEW-STATUS.md once (the hand-maintained Phase 4.5 workflow ledger) and then
leaves it untouched on later runs so manual status edits are never clobbered.
...
PM_STATUS_FILE = REPO_ROOT / "PM-REVIEW-STATUS.md"
...
FEATURE_PILLAR = {
    ...
    "Cloud Amplifier":                      "Pillar 2: Connect & Integrate Data",
    "Combined Schema":                      "Pillar 4: Prepare & Transform Data",
    "Connectors 1.0":                       "Pillar 2: Connect & Integrate Data",
    ...
    "Federated":                            "Pillar 2: Connect & Integrate Data",
    ...
    "Third Party Connectors":               "Pillar 2: Connect & Integrate Data",
    "Workbench":                            "Pillar 2: Connect & Integrate Data",
...
def seed_pm_status_ledger(all_pms: list, all_gaps: list, forum: dict) -> None:
    """Create PM-REVIEW-STATUS.md once (if absent) and NEVER overwrite it afterward.

    This is the hand-maintained workflow ledger for Phase 4.5 — the milestones that
    cannot be derived from the files (brief sent, responses in, sign-offs). It is
    seeded with one row per PM, each workflow cell pre-set to 'todo' where the PM has
    work of that type and '—' where they have none (via collect_pm_tasks). After the
    first run the file is left untouched so human status edits survive every re-run.
    Live open-item counts are deliberately NOT stored here — they live in
    RESTRUCTURE-TASKS.md so they can never drift.
    """
    if PM_STATUS_FILE.exists():
        print(f"  • {PM_STATUS_FILE.relative_to(REPO_ROOT)} exists — left unchanged "
              "(hand-maintained; delete it to re-seed)")
        return
    ...
    header = [
        ...
        "",
        f"_Seeded {date.today().isoformat()} by `scripts/build-pm-review-briefs.py`. **Hand-maintained from here"
        " on** — the script never overwrites this file, so your edits are safe across re-runs. Delete the file"
        " to re-seed a fresh template (e.g. if the PM roster changes)._",
        "",
    ...
    rows = []
    for pm in all_pms:
        t = collect_pm_tasks(pm, all_gaps, forum)
        fc = len(t["fact-check"])
        pmi = len(t["pm-input"])
        life = len(t["legacy"]) + len(t["retired"])
        dec = len(t["decision"])
        total = fc + pmi + life + dec + len(t["update"])
        if total == 0:
            rows.append(f"| {pm} | — | — | — | — | — | — | — |")
            continue
        cell = lambda n: "todo" if n > 0 else "—"
        rows.append(
            f"| {pm} | todo | todo | {cell(fc)} | {cell(pmi)} | {cell(life)} | {cell(dec)} | todo |"
        )
    PM_STATUS_FILE.write_text("\n".join(header + rows) + "\n", encoding="utf-8")
    print(f"  ✓ {PM_STATUS_FILE.relative_to(REPO_ROOT)} seeded (per-PM workflow ledger)")
...
def main():
    ...
    # PM review workflow ledger — seeded once, then hand-maintained (never overwritten)
    seed_pm_status_ledger(all_pms, gaps, forum)
